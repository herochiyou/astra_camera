...
ONI_C_API OniStatus oniWaitForAnyStream(OniStreamHandle *pStreams,
                                        int numStreams, int *pStreamIndex,
                                        int timeout);
 ...
 * formats in which the number of bytes per pixel isn't fixed. */
 ...
 * ONI_DEVICE_PROPERTY_..., or specific ones supplied by the device. */
...
ONI_C_API OniBool oniDeviceSetImageRegistrationMode(
    OniDeviceHandle device, OniImageRegistrationMode mode);
...
/** Get the next frame from the stream. This function is blocking until there is
 * a new frame from the stream. For timeout, use oniWaitForStreams() first */
 ...
 * OniTypes.h: ONI_STREAM_PROPERTY_..., or specific ones supplied by the device
 * for its streams. */
 ...
 * only be called while stream is not started. */
...
ONI_C_API OniStatus oniRegisterAndroidLogRedirectCallback(
    OniAndroidLogRedirectCallback pCallbacks, void *pCookie,
    OniCallbackHandle *pHandle);

ONI_C_API void
oniUnRegisterAndroidLogRedirectCallback(OniCallbackHandle pHandle);
...
// ONI_C_API OniStatus oniConvertRealWorldToProjective(OniStreamHandle stream,
// OniFloatPoint3D* pRealWorldPoint, OniFloatPoint3D* pProjectivePoint);
// ONI_C_API OniStatus oniConvertProjectiveToRealWorld(OniStreamHandle stream,
// OniFloatPoint3D* pProjectivePoint, OniFloatPoint3D* pRealWorldPoint);

/**
 * Creates a recorder that records to a file.
 * @param	[in]	fileName	The name of the file that will contain
 * the recording.
 * @param	[out]	pRecorder	Points to the handle to the newly
 * created recorder.
 ...
 * @param	[in]	recorder				The handle to
 * the recorder.
 * @param	[in]	stream					The handle to
 * the stream.
 ...
 * @param	[in,out]	recorder	The handle to the recorder, the
 *handle will be invalidated (nullified) when the function returns.
 * @retval ONI_STATUS_OK Upon successful completion.
 * @retval ONI_STATUS_ERROR Upon any kind of failure.
 */
 ...
 * @param	OniBool bConsoleOutput	[in]	TRUE to print log entries to
 ...
 * @param	OniBool bFileOutput	[in]	TRUE to print log entries to the
 ...
 * @param	OniBool bAndroidOutput	[in]	TRUE to print log entries to the
 Android log, FALSE otherwise.
 *
 * @retval ONI_STATUS_OK Upon successful completion.
 * @retval ONI_STATUS_ERROR Upon any kind of failure.
 */

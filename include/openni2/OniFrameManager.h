...
typedef void(ONI_CALLBACK_TYPE *BackToPoolFuncPtr)(
    struct OniFrameInternal *pBuffer, void *pCookie);
typedef void(ONI_CALLBACK_TYPE *FreeBufferFuncPtr)(void *data, void *pCookie);

struct OniFrameInternal : public OniFrame {
  int refCount;
  BackToPoolFuncPtr backToPoolFunc; // callback function to be called when frame
                                    // reached zero refs and returned to pool
  void *backToPoolFuncCookie;
  FreeBufferFuncPtr
      freeBufferFunc; // callback function for freeing the frame buffer

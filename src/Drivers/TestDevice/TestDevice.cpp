...
class TestDepthStream : public TestStream {
  public:
  TestDepthStream() : TestStream() { m_frameId = 1; }
...
class TestImageStream : public TestStream {
  public:
  TestImageStream() : TestStream() { m_frameId = 1; }
...
class TestDevice : public oni::driver::DeviceBase {
  ...
  TestDevice(OniDeviceInfo *pInfo, oni::driver::DriverServices &driverServices)
      : m_pInfo(pInfo), m_driverServices(driverServices) {
    m_numSensors = 2;
    ...
    m_sensors[0].pSupportedVideoModes[0].pixelFormat =
  ...
  OniDeviceInfo *GetInfo() { return m_pInfo; }
  ...
  oni::driver::StreamBase *createStream(OniSensorType sensorType) {
    ...
    m_driverServices.errorLoggerAppend(
        "TestDevice: Can't create a stream of type %d", sensorType);
    return NULL;
  }
...
class TestDriver : public oni::driver::DriverBase {
  ...
  virtual oni::driver::DeviceBase *deviceOpen(const char *uri,
                                              const char * /*mode*/) {
    for (xnl::Hash<OniDeviceInfo *, oni::driver::DeviceBase *>::Iterator iter =
             m_devices.Begin();

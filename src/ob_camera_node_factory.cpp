...
void OBCameraNodeFactory::init() {
  ...
  astra_device_lock_shm_ptr_ =
      (uint8_t *)mmap(nullptr, 4, PROT_READ | PROT_WRITE, MAP_SHARED,
                      astra_device_lock_shm_id_, 0);
  pthread_mutexattr_init(&astra_device_lock_attr_);
  pthread_mutexattr_setpshared(&astra_device_lock_attr_,
...
void OBCameraNodeFactory::onDeviceConnected(
  ...
  if (device_info->getUri() == nullptr) {
    ROS_ERROR_STREAM_THROTTLE(1, "Device connected: " << device_info->getName()
                                                      << " uri is null");
    return;
  }
  pthread_mutex_lock(astra_device_lock_);
  std::shared_ptr<int> unlock_guard(
      nullptr, [this](int *) { pthread_mutex_unlock(astra_device_lock_); });
  std::lock_guard<decltype(device_lock_)> lock(device_lock_);
  auto device = std::make_shared<openni::Device>();
  ROS_INFO_STREAM_THROTTLE(1,
                           "Trying to open device: " << device_info->getUri());
  std::this_thread::sleep_for(std::chrono::milliseconds(connection_delay_));
  ROS_INFO_STREAM_THROTTLE(
      1, "OBCameraNodeFactory::onDeviceConnected Open device start");
  auto rc = device->open(device_info->getUri());
  ROS_INFO_STREAM_THROTTLE(
      1,
      "OBCameraNodeFactory::onDeviceConnected Open device done, STATUS " << rc);
  if (rc != openni::STATUS_OK) {
    if (errno == EBUSY) {
      ROS_INFO_STREAM_THROTTLE(1, "Device is already opened OR device is in "
                                  "use, may be it belong to other node");
    } else {
      ROS_ERROR_STREAM_THROTTLE(1, "Failed to open device: "
                                       << device_info->getUri() << " "
                                       << openni::OpenNI::getExtendedError());
    }
  } else {
    ...
    if (rc != openni::STATUS_OK) {
      ROS_ERROR_STREAM("Failed to get serial number: "
               ...
               std::string(serial_number) == serial_number_) {
...
OniLogSeverity
OBCameraNodeFactory::getLogLevelFromString(const std::string &level) {
  if (level == "verbose") {
    ...
    ROS_WARN_STREAM("Unknown log level: " << level
                                          << " oni log will be disable");
    return OniLogSeverity::ONI_LOG_SEVERITY_NONE;
  }
}

...
static const char *ONI_DEFAULT_DRIVERS_REPOSITORY =
    "OpenNI2" XN_FILE_DIR_SEP "Drivers";
...
Context::Context()
    : m_errorLogger(xnl::ErrorLogger::GetInstance()),
      m_initializationCounter(0) {
  xnOSMemSet(m_overrideDevice, 0, XN_FILE_MAX_PATH);
}

Context::~Context() { s_valid = FALSE; }

// Dummy function used only for taking its address for the sake of
// xnOSGetModulePathForProcAddress.
...
OniStatus Context::initialize() {
  ...
  rc = xnOSGetModulePathForProcAddress(
      reinterpret_cast<void *>(&dummyFunctionToTakeAddress), strModulePath);
  if (rc != XN_STATUS_OK) {
    m_errorLogger.Append(
        "Couldn't get the OpenNI shared library module's path: %s",
  ...
  if (rc != XN_STATUS_OK) {
    ...
    m_errorLogger.Append(
        "Couldn't get the OpenNI shared library module's directory: %s",
        xnGetStatusString(rc));
    return OniStatusFromXnStatus(rc);
  }
  ...
  rc = xnOSAppendFilePath(strOniConfigurationFile, ONI_CONFIGURATION_FILE,
                          XN_FILE_MAX_PATH);
  if (rc == XN_STATUS_OK) {
  ...
  if (configurationFileExists) {
    // First, we should process the log related configuration as early as
    // possible.
    ...
    rc = xnOSReadStringFromINI(strOniConfigurationFile, "Log", "LogPath",
                               strLogPath, XN_FILE_MAX_PATH);
    if (rc == XN_STATUS_OK) {
      rc = xnLogSetOutputFolder(strLogPath);
      if (rc != XN_STATUS_OK) {
        xnLogWarning(XN_MASK_ONI_CONTEXT, "Failed to set log output folder: %s",
                     xnGetStatusString(rc));
      } else {
        xnLogVerbose(XN_MASK_ONI_CONTEXT, "Log directory redirected to: %s",
                     strLogPath);
      }
    }

    rc = xnOSReadIntFromINI(strOniConfigurationFile, "Log", "Verbosity",
                            &nValue);
    if (rc == XN_STATUS_OK) {
      xnLogSetMaskMinSeverity(XN_LOG_MASK_ALL, (XnLogSeverity)nValue);
    }

    rc = xnOSReadIntFromINI(strOniConfigurationFile, "Log", "LogToConsole",
                            &nValue);
    if (rc == XN_STATUS_OK) {
      xnLogSetConsoleOutput(nValue == 1);
    }

    rc = xnOSReadIntFromINI(strOniConfigurationFile, "Log", "LogToFile",
                            &nValue);
    if (rc == XN_STATUS_OK) {
    ...
    rc = xnOSReadStringFromINI(strOniConfigurationFile, "Device", "Override",
                               m_overrideDevice, XN_FILE_MAX_PATH);
    if (rc != XN_STATUS_OK) {
      xnLogVerbose(XN_MASK_ONI_CONTEXT,
                   "No override device in configuration file");
    }

    rc = xnOSReadStringFromINI(strOniConfigurationFile, "Drivers", "Repository",
                               repositoryFromINI, XN_FILE_MAX_PATH);
    if (rc == XN_STATUS_OK) {
      repositoryOverridden = TRUE;
    }

    xnLogVerbose(XN_MASK_ONI_CONTEXT, "Configuration has been read from '%s'",
                 strOniConfigurationFile);
  } else {
    xnLogVerbose(XN_MASK_ONI_CONTEXT, "Couldn't find configuration file '%s'",
                 strOniConfigurationFile);
  }
  ...
  if (repositoryOverridden) {
    xnLogVerbose(
        XN_MASK_ONI_CONTEXT,
        "Extending the driver path by '%s', as configured in file '%s'",
        repositoryFromINI, strOniConfigurationFile);
    rc = xnOSAppendFilePath(strDriverPath, repositoryFromINI, XN_FILE_MAX_PATH);
  } else {
    rc = xnOSAppendFilePath(strDriverPath, ONI_DEFAULT_DRIVERS_REPOSITORY,
                            XN_FILE_MAX_PATH);
  }
...
XnStatus Context::loadLibraries(const char *directoryName) {
  ...
  xnLogVerbose(XN_MASK_ONI_CONTEXT,
               "Looking for drivers in drivers repository '%s'", directoryName);

  // Build the search pattern string
  XN_VALIDATE_STR_APPEND(cpSearchString, directoryName, XN_FILE_MAX_PATH,
                         nRetVal);
  XN_VALIDATE_STR_APPEND(cpSearchString, XN_FILE_DIR_SEP, XN_FILE_MAX_PATH,
                         nRetVal);
  XN_VALIDATE_STR_APPEND(cpSearchString, XN_SHARED_LIBRARY_PREFIX,
                         XN_FILE_MAX_PATH, nRetVal);
  XN_VALIDATE_STR_APPEND(cpSearchString, XN_FILE_ALL_WILDCARD, XN_FILE_MAX_PATH,
                         nRetVal);
  XN_VALIDATE_STR_APPEND(cpSearchString, XN_SHARED_LIBRARY_POSTFIX,
                         XN_FILE_MAX_PATH, nRetVal);

  nRetVal = xnOSCountFiles(cpSearchString, &nFileCount);
  if (nRetVal != XN_STATUS_OK || nFileCount == 0) {
    xnLogError(XN_MASK_ONI_CONTEXT, "Found no drivers matching '%s'",
               cpSearchString);
  ...
  nRetVal = xnOSGetFileList(cpSearchString, NULL, acsFileList, nFileCount,
                            &nFileCount);
#else
  // Android
  nFileCount = 3;
  ...
  for (int i = 0; i < nFileCount; ++i) {
    DeviceDriver *pDeviceDriver =
        XN_NEW(DeviceDriver, acsFileList[i], m_frameManager, m_errorLogger);
    if (pDeviceDriver == NULL || !pDeviceDriver->isValid()) {
      xnLogVerbose(XN_MASK_ONI_CONTEXT,
                   "Couldn't use file '%s' as a device driver", acsFileList[i]);
      m_errorLogger.Append("Couldn't understand file '%s' as a device driver",
                           acsFileList[i]);
      XN_DELETE(pDeviceDriver);
      continue;
    }
    OniCallbackHandle dummy;
    pDeviceDriver->registerDeviceConnectedCallback(deviceDriver_DeviceConnected,
                                                   this, dummy);
    pDeviceDriver->registerDeviceDisconnectedCallback(
        deviceDriver_DeviceDisconnected, this, dummy);
    pDeviceDriver->registerDeviceStateChangedCallback(
        deviceDriver_DeviceStateChanged, this, dummy);
    if (!pDeviceDriver->initialize()) {
      xnLogVerbose(XN_MASK_ONI_CONTEXT,
                   "Couldn't use file '%s' as a device driver", acsFileList[i]);
      m_errorLogger.Append("Couldn't initialize device driver from file '%s'",
                           acsFileList[i]);
      XN_DELETE(pDeviceDriver);
      continue;
    }
...
void Context::shutdown() {
  ...
  if (m_initializationCounter > 0) {
    xnLogInfo(XN_MASK_ONI_CONTEXT,
              "Shutdown: still need %d more shutdown calls (to match "
              "initializations)",
              m_initializationCounter);
    return;
  }
  ...
  for (xnl::List<DeviceDriver *>::Iterator iter = m_deviceDrivers.Begin();
       iter != m_deviceDrivers.End(); ++iter) {
...
OniStatus Context::registerDeviceConnectedCallback(
    OniDeviceInfoCallback handler, void *pCookie, OniCallbackHandle &handle) {
  return OniStatusFromXnStatus(m_deviceConnectedEvent.Register(
      handler, pCookie, (XnCallbackHandle &)handle));
}
...
OniStatus Context::registerDeviceDisconnectedCallback(
    OniDeviceInfoCallback handler, void *pCookie, OniCallbackHandle &handle) {
  return OniStatusFromXnStatus(m_deviceDisconnectedEvent.Register(
      handler, pCookie, (XnCallbackHandle &)handle));
}
...
OniStatus Context::registerDeviceStateChangedCallback(
    OniDeviceStateCallback handler, void *pCookie, OniCallbackHandle &handle) {
  return OniStatusFromXnStatus(m_deviceStateChangedEvent.Register(
      handler, pCookie, (XnCallbackHandle &)handle));
}
...
OniStatus Context::getDeviceList(OniDeviceInfo **pDevices, int *pDeviceCount) {
  ...
  *pDevices = XN_NEW_ARR(OniDeviceInfo, *pDeviceCount);
  ...
  m_cs.Unlock();
  LOG(ERROR) << "Device count:(" << m_devices.Size() << ")3.";
  return ONI_STATUS_OK;
}
...
OniStatus Context::deviceOpen(const char *uri, const char *mode,
  ...
  xnLogVerbose(XN_MASK_ONI_CONTEXT, "Trying to open device by URI '%s'",
               deviceURI == NULL ? "(NULL)" : deviceURI);
  ...
  if (deviceURI == NULL) {
    ...
    for (xnl::List<Device *>::Iterator iter = m_devices.Begin();
  ...
  if (pMyDevice == NULL) {
    for (xnl::List<DeviceDriver *>::Iterator iter = m_deviceDrivers.Begin();
         iter != m_deviceDrivers.End() && pMyDevice == NULL; ++iter) {
...
const OniSensorInfo *Context::getSensorInfo(OniDeviceHandle device,
...
OniStatus Context::createStream(OniDeviceHandle device,
  ...
  if (pMyStream == NULL) {
    m_errorLogger.Append(
        "Context: Couldn't create stream from device:%08x, source: %d", device,
        sensorType);
    return ONI_STATUS_ERROR;
  }
  ...
  StreamFrameHolder *pFrameHolder =
      XN_NEW(StreamFrameHolder, m_frameManager, pMyStream);
  if (pFrameHolder == NULL) {
    m_errorLogger.Append("Context: Couldn't create stream frame holder from "
                         "device:%08x, source: %d",
                         device, sensorType);
    XN_DELETE(pMyStream);
    return ONI_STATUS_ERROR;
...
OniStatus Context::streamDestroy(VideoStream *pStream) {
  ...
  for (int i = 0; i < numStreams; ++i) {
    if (pStreamList[i] != pStream) {
      ...
      StreamFrameHolder *pStreamFrameHolder =
          XN_NEW(StreamFrameHolder, m_frameManager, pStreamList[i]);
...
void Context::frameRelease(OniFrame *pFrame) { m_frameManager.release(pFrame); }

void Context::frameAddRef(OniFrame *pFrame) { m_frameManager.addRef(pFrame); }

OniStatus Context::waitForStreams(OniStreamHandle *pStreams, int streamCount,
                                  int *pStreamIndex, int timeout) {
  ...
  if (streamCount > MAX_WAITED_STREAMS) {
    m_errorLogger.Append("Cannot wait on more than %d streams",
                         MAX_WAITED_STREAMS);
...
OniStatus Context::enableFrameSync(OniStreamHandle *pStreams, int numStreams,
                                   OniFrameSyncHandle *pFrameSyncHandle) {
  // Verify parameters.
  ...
  for (int i = 0; i < numStreams; ++i) {
    // Make sure stream's device is valid and is same as device of other
    // streams.
    if (pDeviceDriver == NULL) {
      ...
      if (pDeviceDriver !=
          pStreams[i]->pStream->getDevice().getDeviceDriver()) {
        // Frame sync groups using streams from different drivers is not
        // supported.
        m_errorLogger.Append(
            "EnableFrameSync: can't sync streams from different drivers");
        return ONI_STATUS_NOT_SUPPORTED;
      }
    }
  ...
  return enableFrameSyncEx(pStreamList.GetData(), numStreams, pDeviceDriver,
                           pFrameSyncHandle);
}

OniStatus Context::enableFrameSyncEx(VideoStream **pStreams, int numStreams,
                                     DeviceDriver *pDeviceDriver,
                                     OniFrameSyncHandle *pFrameSyncHandle) {
  ...
  SyncedStreamsFrameHolder *pSyncedStreamsFrameHolder =
      XN_NEW(SyncedStreamsFrameHolder, m_frameManager, pStreams, numStreams);
...
void Context::disableFrameSync(OniFrameSyncHandle frameSyncHandle) {
  ...
  frameSyncHandle->pDeviceDriver->disableFrameSync(
      frameSyncHandle->pFrameSyncHandle);
  ...
  xnl::Array<VideoStream *> pStreamList(numStreams);
  pStreamList.SetSize(numStreams);
  frameSyncHandle->pSyncedStreamsFrameHolder->getStreams(pStreamList.GetData(),
                                                         &numStreams);
...
void ONI_CALLBACK_TYPE Context::deviceDriver_DeviceConnected(Device *pDevice,
                                                             void *pCookie) {
...
void ONI_CALLBACK_TYPE Context::deviceDriver_DeviceDisconnected(Device *pDevice,
                                                                void *pCookie) {
  Context *pContext = (Context *)pCookie;
...
void ONI_CALLBACK_TYPE Context::deviceDriver_DeviceStateChanged(
    Device *pDevice, OniDeviceState deviceState, void *pCookie) {
  Context *pContext = (Context *)pCookie;
  pContext->m_deviceStateChangedEvent.Raise(pDevice->getInfo(), deviceState);
}

OniStatus Context::recorderOpen(const char *fileName,
  ...
  *pRecorder = XN_NEW(_OniRecorder);
  ...
  if (NULL == ((*pRecorder)->pRecorder = XN_NEW(Recorder, m_frameManager,
                                                m_errorLogger, *pRecorder))) {
    XN_DELETE(*pRecorder);
    return ONI_STATUS_ERROR;
  }
...
void Context::clearErrorLogger() { m_errorLogger.Clear(); }
...
void Context::onNewFrame() {
  m_cs.Lock();
  for (xnl::Hash<XN_THREAD_ID, XN_EVENT_HANDLE>::Iterator it =
           m_waitingThreads.Begin();

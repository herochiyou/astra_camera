...
RecordAssembler::RecordAssembler()
    : m_pBuffer(NULL), m_bufferSize_bytes(0), m_pEmitPtr(NULL) {}
...
#define MUST_BE_INITIALIZED(...)                                               \
  if (0 == m_bufferSize_bytes) {                                               \
    return __VA_ARGS__;                                                        \
  } else {                                                                     \
  }

void RecordAssembler::emitCommonHeader(XnUInt32 recordType, XnUInt32 nodeId,
...
OniStatus RecordAssembler::emitString(const XnChar *pStr,
                                      XnSizeT &totalFieldsSize_bytes) {
...
template <typename T>
OniStatus RecordAssembler::emit(const T &field,
                                XnSizeT &totalFieldsSize_bytes) {
  MUST_BE_INITIALIZED(ONI_STATUS_ERROR)
...
OniStatus RecordAssembler::emit_RECORD_NODE_ADDED_1_0_0_5(
    XnUInt32 nodeType, XnUInt32 nodeId, XnUInt32 codecId,
    XnUInt32 numberOfFrames, XnUInt64 minTimeStamp, XnUInt64 maxTimeStamp) {
  MUST_BE_INITIALIZED(ONI_STATUS_ERROR)
  ...
  // Fields of v. 1.0.0.5 record (size & offset are in bytes [decimal
  // notation]): Size     Offset      Fields                    Notes
  // ====     ======      ======                    =====
  //           0          +-------------------+
  //   4              +-->| Size of Node Name
  //   |\                                /
...
OniStatus RecordAssembler::emit_RECORD_NODE_ADDED(
  ...
  OniStatus status = emit_RECORD_NODE_ADDED_1_0_0_5(
      nodeType, nodeId, codecId, numberOfFrames, minTimeStamp, maxTimeStamp);
  ...
  //
  // Fields of v. 1.0.0.6 record (size & offset are in bytes [decimal
  // notation]): Size     Offset      Fields                    Notes
  // ====     ======      ======                    =====
  //                      +---------------------+
  //                      | V. 1.0.0.5 Fields   |
...
OniStatus RecordAssembler::emit_RECORD_NODE_REMOVED(XnUInt32 nodeId,
                                                    XnUInt64 nodeAddedPos) {
  MUST_BE_INITIALIZED(ONI_STATUS_ERROR)
...
OniStatus
RecordAssembler::emit_RECORD_SEEK_TABLE(XnUInt32 nodeId, XnUInt32 numFrames,
  ...
  emitData(&emptyEntry, entrySize);
  ++nWritten;

  // now the table itself
  for (DataIndexEntryList::ConstIterator it = dataIndexEntryList.Begin();
       it != dataIndexEntryList.End(); ++it) {
    emitData(&(*it), entrySize);
    ++nWritten;
  }
...
OniStatus RecordAssembler::emit_RECORD_END() {
  MUST_BE_INITIALIZED(ONI_STATUS_ERROR)
  emitCommonHeader(RECORD_END, /* nodeId (ignored) = */ XN_UINT32_C(0),
                   /*undoRecordPos=*/0);
  return ONI_STATUS_OK;
}

OniStatus RecordAssembler::emit_RECORD_NODE_DATA_BEGIN(XnUInt32 nodeId,
  ...
  // Fields of DATA_BEGIN record (size & offset are in bytes [decimal
  // notation]): Size     Offset      Fields                    Notes
  // ====     ======      ======                    =====
  //           0          +------------------+
  //   4                  | Frames Count     |
...
OniStatus RecordAssembler::emit_RECORD_NEW_DATA(
    XnUInt32 nodeId, XnUInt64 undoRecordPos, XnUInt64 timeStamp,
    XnUInt32 frameId, const void *data, XnSizeT dataSize_bytes) {
  MUST_BE_INITIALIZED(ONI_STATUS_ERROR)
...
OniStatus RecordAssembler::emit_RECORD_GENERAL_PROPERTY(
    XnUInt32 nodeId, XnUInt64 undoRecordPos, const char *propertyName,
    const void *data, XnSizeT dataSize_bytes) {
  MUST_BE_INITIALIZED(ONI_STATUS_ERROR)
  ...
  //
  // Fields of GENERAL_PROPERTY record (size & offset are in bytes [decimal
  // notation]): Size     Offset      Fields                    Notes
  // ====     ======      ======                    =====
  //           0          +-----------------------+
...
OniStatus RecordAssembler::emit_RECORD_INT_PROPERTY(XnUInt32 nodeId,
                                                    XnUInt64 undoRecordPos,
  ...
  OniStatus status = emit_RECORD_GENERAL_PROPERTY(
      nodeId, undoRecordPos, propertyName, &data, sizeof(data));
  if (ONI_STATUS_OK == status) {
...
OniStatus RecordAssembler::emit_RECORD_REAL_PROPERTY(XnUInt32 nodeId,
                                                     XnUInt64 undoRecordPos,
                                                     const char *propertyName,
                                                     XnDouble data) {
  MUST_BE_INITIALIZED(ONI_STATUS_ERROR)

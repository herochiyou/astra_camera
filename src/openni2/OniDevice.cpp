...
Device::Device(DeviceDriver *pDeviceDriver, const DriverHandler &driverHandler,
               FrameManager &frameManager, const OniDeviceInfo *pDeviceInfo,
               xnl::ErrorLogger &errorLogger)
    : m_driverHandler(driverHandler), m_frameManager(frameManager),
      m_errorLogger(errorLogger), m_active(false), m_openCount(0),
      m_deviceHandle(NULL), m_pDeviceDriver(pDeviceDriver),
      m_depthColorSyncHandle(NULL), m_pContext(NULL), m_syncEnabled(FALSE) {
...
OniStatus Device::getSensorInfoList(OniSensorInfo **pSensorInfos,
                                    int &numSensors) {
  return m_driverHandler.deviceGetSensorInfoList(m_deviceHandle, pSensorInfos,
                                                 &numSensors);
}
...
VideoStream *Device::createStream(OniSensorType sensorType) {
  ...
  if (m_sensors[sensorType] == NULL) {
    m_sensors[sensorType] =
        XN_NEW(Sensor, m_errorLogger, m_frameManager, m_driverHandler);
    if (m_sensors[sensorType] == NULL) {
      XN_ASSERT(FALSE);
      return NULL;
    }
...

  {
    ...
    if (m_sensors[sensorType]->m_streamCount == 0) {
      void *streamHandle =
          m_driverHandler.deviceCreateStream(m_deviceHandle, sensorType);
      if (streamHandle == NULL) {
        m_errorLogger.Append("Stream: couldn't create using source %d",
                             sensorType);
        return NULL;
      }
  ...
  VideoStream *pStream =
      XN_NEW(VideoStream, m_sensors[sensorType], pSensorInfo, *this,
             m_driverHandler, m_frameManager, m_errorLogger);
...
OniStatus oni::implementation::Device::setProperty(int propertyId,
                                                   const void *data,
                                                   int dataSize) {
  OniStatus rc = m_driverHandler.deviceSetProperty(m_deviceHandle, propertyId,
                                                   data, dataSize);
  if (rc != ONI_STATUS_OK) {
...
OniStatus oni::implementation::Device::getProperty(int propertyId, void *data,
                                                   int *pDataSize) {
  OniStatus rc = m_driverHandler.deviceGetProperty(m_deviceHandle, propertyId,
                                                   data, pDataSize);
  if (rc != ONI_STATUS_OK) {
...
OniStatus Device::invoke(int commandId, void *data, int dataSize) {
  ...
  return m_driverHandler.deviceInvoke(m_deviceHandle, commandId, data,
                                      dataSize);
}
...
OniStatus Device::enableDepthColorSync(Context *pContext) {
  ...
  int streamsUsed = 0;
  for (xnl::List<VideoStream *>::Iterator iter = m_streams.Begin();
       iter != m_streams.End(); ++iter) {
    if (((*iter)->getSensorInfo()->sensorType == ONI_SENSOR_DEPTH ||
         (*iter)->getSensorInfo()->sensorType == ONI_SENSOR_COLOR) &&
        (*iter)->isStarted()) {
  ...
  return m_pContext->enableFrameSyncEx(streamArray.GetData(), streamsUsed,
                                       m_pDeviceDriver,
                                       &m_depthColorSyncHandle);
}
...
OniBool Device::isDepthColorSyncEnabled() { return m_syncEnabled; }

void ONI_CALLBACK_TYPE Device::stream_PropertyChanged(void * /*deviceHandle*/,
                                                      int /*propertyId*/,
                                                      const void * /*data*/,
                                                      int /*dataSize*/,
                                                      void *pCookie) {
...
OniBool
Device::isImageRegistrationModeSupported(OniImageRegistrationMode mode) {
  return m_driverHandler.deviceIsImageRegistrationModeSupported(m_deviceHandle,
                                                                mode);
}

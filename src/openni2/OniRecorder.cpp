...
const XnSizeT propertyTableItemsCount =
    sizeof(propertyTable) / sizeof(propertyTable[0]);
...
class Recorder::Memento {
   ...
   */
  void Release() { m_needRollback = false; }
  ...
  /**
   * An easy way to get the current position of the file. (and save for later
   * use)
   */
  XnUInt64 GetPosition() { return m_offset; }
...
Recorder::Recorder(FrameManager &frameManager, xnl::ErrorLogger &errorLogger,
                   OniRecorderHandle handle)
    : m_frameManager(frameManager), m_errorLogger(errorLogger),
      m_handle(handle), m_maxId(0), m_configurationId(0),
      m_propertyPriority(ms_priorityNormal), m_file(XN_INVALID_FILE_HANDLE),
      m_running(FALSE), m_started(FALSE), m_wasStarted(FALSE) {}
...
OniStatus Recorder::attachStream(VideoStream &stream,
...
OniStatus Recorder::detachAllStreams() {
  xnl::LockGuard<AttachedStreams> guard(m_streams);
  for (AttachedStreams::Iterator i = m_streams.Begin(), e = m_streams.End();
       i != e; ++i) {
    detachStream(*i->Key());
  }
  return ONI_STATUS_OK;
...
void Recorder::stop() { m_started = false; }
...
OniStatus Recorder::recordStreamProperty(VideoStream &stream, int propertyId,
  ...
  send(Message::MESSAGE_RECORDPROPERTY, pStream, newPtr, propertyId, dataSize,
       m_propertyPriority);
  return ONI_STATUS_OK;
}
  ...
  if (XN_STATUS_OK == nRetVal) {
    ...
    case Message::MESSAGE_RECORD: {
      ...
      if (i != m_streams.End()) {
        ...
        if (frameId > 1) {
          timestamp = m_streams[msg.pStream].lastOutputTimestamp +
                      (msg.pFrame->timestamp -
                       m_streams[msg.pStream].lastInputTimestamp);
        }
    ...
    case Message::MESSAGE_RECORDPROPERTY: {
      ...
      AttachedStreams::Iterator i = m_streams.Find(msg.pStream);
      if (i != m_streams.End()) {
        onRecordProperty(i->Value().nodeId, msg.propertyId, msg.pData,
                         msg.dataSize);
      }
...
void Recorder::send(Message::Type type, VideoStream *pStream, const void *pData,
                    XnUInt32 propertyId, XnSizeT dataSize, int priority) {
  Message msg = {type,       pStream != NULL ? m_streams[pStream].nodeId : 0,
                 pStream,    {pData},
                 propertyId, dataSize};
  xnl::LockGuard<MessageQueue> guard(m_queue);
...
XnUInt64 Recorder::getLastPropertyRecordPos(XnUInt32 nodeId,
                                            const char *propName,
                                            XnUInt64 newRecordPos) {
  XnUInt64 pos = 0;
  FIND_ATTACHED_STREAM_INFO(nodeId)
  if (!pInfo)
    return 0;

  if (pInfo->lastPropertyRecordPosition.Find(propName) !=
      pInfo->lastPropertyRecordPosition.End()) {
...
void fillXnSupportedPixelFormats(XnSupportedPixelFormats &xnSPF,
                                 OniPixelFormat oniFormat) {
  ...
  case XN_PIXEL_FORMAT_GRAYSCALE_8_BIT:
    xnSPF.m_bGrayscale8Bit = 1;
    break;
  case XN_PIXEL_FORMAT_GRAYSCALE_16_BIT:
    xnSPF.m_bGrayscale16Bit = 1;
    break;
...
void Recorder::onAttach(XnUInt32 nodeId, VideoStream *pStream) {
  ...
  case ONI_PIXEL_FORMAT_DEPTH_1_MM: {
    ...
    m_streams[pStream].pCodec =
        XN_NEW(Xn16zEmbTablesCodec, static_cast<XnUInt16>(maxDepth));
  ...
  case ONI_PIXEL_FORMAT_RGB888: {
    if (m_streams[pStream].allowLossyCompression) {
      m_streams[pStream].pCodec =
          XN_NEW(XnJpegCodec,
                 /* bRGB = */ TRUE, curVideoMode.resolutionX,
  ...
  EMIT(RECORD_NODE_ADDED(m_streams[pStream].nodeType =
                             AsNodeType(pSensorInfo->sensorType),
                         nodeId, m_streams[pStream].codecId = codecId,
  ...
  EMIT(RECORD_INT_PROPERTY(
      nodeId,
      getLastPropertyRecordPos(nodeId, "oniRequiredFrameSize",
                               undoPoint.GetPosition()),
      "oniRequiredFrameSize", pStream->getRequiredFrameSize()));
  undoPoint.Reuse();

  // isGenerating (needed for OpenNI 1.x playback)
  EMIT(RECORD_INT_PROPERTY(nodeId,
                           getLastPropertyRecordPos(nodeId, "xnIsGenerating",
                                                    undoPoint.GetPosition()),
                           "xnIsGenerating", TRUE));
  undoPoint.Reuse();
  ...
  if (curVideoMode.pixelFormat == ONI_PIXEL_FORMAT_DEPTH_1_MM ||
    ...
    EMIT(
        RECORD_INT_PROPERTY(nodeId,
                            getLastPropertyRecordPos(nodeId, "xnDeviceMaxDepth",
                                                     undoPoint.GetPosition()),
                            "xnDeviceMaxDepth", maxDepth))
  }
  undoPoint.Reuse();
  ...
  EMIT(RECORD_INT_PROPERTY(
      nodeId,
      getLastPropertyRecordPos(nodeId, "xnSupportedMapOutputModesCount",
                               undoPoint.GetPosition()),
      "xnSupportedMapOutputModesCount", pSensorInfo->numSupportedVideoModes))
  undoPoint.Reuse();

  // xnSupportedMapOutputModes
  VideoModeData *pVideoModes =
      XN_NEW_ARR(VideoModeData, pSensorInfo->numSupportedVideoModes);
  ...
  EMIT(RECORD_GENERAL_PROPERTY(
      nodeId,
      getLastPropertyRecordPos(nodeId, "xnSupportedMapOutputModes",
                               undoPoint.GetPosition()),
      "xnSupportedMapOutputModes", pVideoModes,
  ...
  EMIT(RECORD_GENERAL_PROPERTY(
      nodeId,
      getLastPropertyRecordPos(nodeId, "xnMapOutputMode",
                               undoPoint.GetPosition()),
      "xnMapOutputMode", &curVMD, sizeof(curVMD)))
  undoPoint.Reuse();
  ...
  if (pixelFormat != 0) {
    ...
    fillXnSupportedPixelFormats(supportedPixelFormats,
                                curVideoMode.pixelFormat);
    EMIT(RECORD_GENERAL_PROPERTY(
        nodeId,
        getLastPropertyRecordPos(nodeId, "xnSupportedPixelFormats",
                                 undoPoint.GetPosition()),
        "xnSupportedPixelFormats", &supportedPixelFormats,
    ...
    // xnPixelFormat
    EMIT(RECORD_INT_PROPERTY(nodeId,
                             getLastPropertyRecordPos(nodeId, "xnPixelFormat",
                                                      undoPoint.GetPosition()),
  ...
  EMIT(RECORD_INT_PROPERTY(nodeId,
                           getLastPropertyRecordPos(nodeId, "oniPixelFormat",
                                                    undoPoint.GetPosition()),
                           "oniPixelFormat", curVideoMode.pixelFormat))
  undoPoint.Reuse();
  ...
  size = sizeof(XnFloat);
  float vdummy, hdummy;
  if (pStream->getProperty(ONI_STREAM_PROPERTY_HORIZONTAL_FOV, &hdummy,
                           &size) == ONI_STATUS_OK &&
      pStream->getProperty(ONI_STREAM_PROPERTY_VERTICAL_FOV, &vdummy, &size) ==
          ONI_STATUS_OK) {
    // xnFOV
  ...
  if (pStream->getProperty(ONI_STREAM_PROPERTY_CROPPING, &cropping, &size) ==
      ONI_STATUS_OK) {
    ...
    EMIT(RECORD_GENERAL_PROPERTY(
        ...
        "xnCropping", &xncropping, sizeof(xncropping)))
  ...
  size = sizeof(bMirror);
  if (pStream->getProperty(ONI_STREAM_PROPERTY_MIRRORING, &bMirror, &size) ==
      ONI_STATUS_OK) {
...
void Recorder::onDetach(XnUInt32 nodeId) {
  if (nodeId == 0) {
    return;
  }
  ...
  Memento undoPoint(this);
  EMIT(RECORD_NODE_REMOVED(nodeId, pInfo->nodeAddedRecordPosition))
  undoPoint.Release();

  undoPoint.Reuse();
  XnUInt64 nSeekTablePos = undoPoint.GetPosition();
  // write the seek table
  EMIT(RECORD_SEEK_TABLE(nodeId, pInfo->frameId, pInfo->dataIndex))
  ...
  EMIT(RECORD_NODE_ADDED(pInfo->nodeType, nodeId, pInfo->codecId,
...
void Recorder::onStart(XnUInt32 nodeId) {
  if (0 == nodeId) {
    return;
  }
  Memento undoPoint(this);
  EMIT(RECORD_NODE_STATE_READY(nodeId))

  EMIT(RECORD_NODE_DATA_BEGIN(nodeId,
                              /* framesCount  = */ XN_MAX_UINT32,
                              /* maxTimeStamp = */ XN_MAX_UINT64))
  undoPoint.Release();
}

void Recorder::onRecord(XnUInt32 nodeId, XnCodecBase *pCodec,
                        const OniFrame *pFrame, XnUInt32 frameId,
                        XnUInt64 timestamp) {
  ...
  if (NULL != pCodec) {
    XnUInt32 bufferSize_bytes32 =
        pFrame->dataSize * 2 + pCodec->GetOverheadSize();
    XnUInt8 *buffer = XN_NEW_ARR(XnUInt8, bufferSize_bytes32);

    XnStatus status =
        pCodec->Compress(reinterpret_cast<const XnUChar *>(pFrame->data),
                         pFrame->dataSize, buffer, &bufferSize_bytes32);
    XnSizeT bufferSize_bytes = bufferSize_bytes32;
    if (XN_STATUS_OK == status) {
      EMIT(RECORD_NEW_DATA(nodeId, pInfo->lastNewDataRecordPosition, timestamp,
                           frameId, buffer, bufferSize_bytes))
    }
    XN_DELETE_ARR(buffer);
  } else {
    EMIT(RECORD_NEW_DATA(nodeId, pInfo->lastNewDataRecordPosition,
                         pFrame->timestamp, pFrame->frameIndex, pFrame->data,
                         pFrame->dataSize))
  }
  undoPoint.Release();
...
void Recorder::onRecordProperty(XnUInt32 nodeId, XnUInt32 propertyId,
                                const void *pData, XnSizeT dataSize) {
  ...
  for (XnSizeT i = 0; i < propertyTableItemsCount; ++i) {
    if (propertyTable[i].propertyId == propertyId) {
      ...
      case PROPERTY_TYPE_INTEGER: {
        ...
        EMIT(RECORD_INT_PROPERTY(
            nodeId,
            getLastPropertyRecordPos(nodeId, propertyTable[i].propertyName,
                                     undoPoint.GetPosition()),
            propertyTable[i].propertyName, value))
      } break;
      ...
      default: {
        EMIT(RECORD_GENERAL_PROPERTY(
            ...
            getLastPropertyRecordPos(nodeId, propertyTable[i].propertyName,
                                     undoPoint.GetPosition()),
            propertyTable[i].propertyName, pData, dataSize))
      }
      }
    }
  }

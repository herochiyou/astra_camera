...
class Recorder {
   ...
   */
  Recorder(FrameManager &frameManager, xnl::ErrorLogger &errorLogger,
           OniRecorderHandle handle = NULL);
   ...
   *
   */
  OniStatus recordStreamProperty(VideoStream &stream, int propertyId,
  ...
  // Message::MESSAGE_DETACH relies only upon the nodeId field of the message,
  // other fields are invalid and irrelevant for that message.
  struct Message {
    enum Type {
      MESSAGE_NO_OPERATION, ///< Does not use any of Message fields. Does
                            ///< nothing.
  ...
  void send(Message::Type type, VideoStream *pStream = NULL,
            const void *pData = NULL, XnUInt32 propertyId = 0u,
            XnSizeT dataSize = 0u, int priority = ms_priorityNormal);
  ...
  void onRecord(XnUInt32 nodeId, XnCodecBase *pCodec, const OniFrame *pFrame,
                XnUInt32 frameId, XnUInt64 timestamp);
  void onRecordProperty(XnUInt32 nodeId, XnUInt32 propertyId, const void *pData,
                        XnSizeT dataSize);
  ...
  // There's a 1:1 mapping between a Recorder object and a handle to that
  // object. The Recorder object owns its handle.
  ...
  typedef xnl::Lockable<xnl::Hash<VideoStream *, AttachedStreamInfo>>
      AttachedStreams;
  AttachedStreams m_streams;

  // A helper function for the properties' undoRecordPos
  XnUInt64 getLastPropertyRecordPos(XnUInt32 nodeId, const char *propName,
  ...
  // to avoid the player seeking with the tables when a configuration has
  // changed.

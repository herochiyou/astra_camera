...
Sensor::Sensor(xnl::ErrorLogger &errorLogger, FrameManager &frameManager,
               const DriverHandler &driverHandler)
    : m_streamCount(0), m_startedStreamCount(0), m_errorLogger(errorLogger),
      m_frameManager(frameManager), m_driverHandler(driverHandler),
      m_streamHandle(NULL), m_requiredFrameSize(0) {
  resetFrameAllocator();

  OniStreamServices::streamServices = this;
  OniStreamServices::getDefaultRequiredFrameSize =
      getDefaultRequiredFrameSizeCallback;
...
Sensor::~Sensor() { releaseAllFrames(); }

void Sensor::setDriverStream(void *streamHandle) {
  m_streamHandle = streamHandle;
  m_driverHandler.streamSetServices(m_streamHandle, this);
  m_driverHandler.streamSetNewFrameCallback(m_streamHandle, newFrameCallback,
                                            this);
}

OniStatus Sensor::setFrameBufferAllocator(OniFrameAllocBufferCallback alloc,
                                          OniFrameFreeBufferCallback free,
                                          void *pCookie) {
  xnl::AutoCSLocker lock(m_refCountCS);
  if (m_startedStreamCount > 0) {
    m_errorLogger.Append(
        "Cannot set frame buffer allocator while stream is running");
  ...
  if (alloc == NULL && free == NULL) {
    ...
    m_errorLogger.Append(
        "Cannot set only alloc or only free function. Both must be supplied.");
...
OniFrame *Sensor::acquireFrame() {
  ...
  pResult->data = m_allocFrameBufferCallback(m_requiredFrameSize,
                                             m_frameBufferAllocatorCookie);
...
void *ONI_CALLBACK_TYPE
Sensor::allocFrameBufferFromPoolCallback(int size, void *pCookie) {
  Sensor *pThis = (Sensor *)pCookie;
  return pThis->allocFrameBufferFromPool(size);
}

void ONI_CALLBACK_TYPE Sensor::releaseFrameBufferToPoolCallback(void *pBuffer,
                                                                void *pCookie) {
  Sensor *pThis = (Sensor *)pCookie;
...
void ONI_CALLBACK_TYPE
Sensor::freeFrameBufferMemoryCallback(void *pBuffer, void * /*pCookie*/) {
...
void Sensor::releaseAllFrames() {
  ...
  for (xnl::List<OniFrameInternal *>::Iterator it =
           m_currentStreamFrames.Begin();
       it != m_currentStreamFrames.End(); ++it) {
  ...
  for (xnl::List<void *>::Iterator it = m_availableFrameBuffers.Begin();
       it != m_availableFrameBuffers.End(); ++it) {
    xnOSFreeAligned(*it);
  }
  m_availableFrameBuffers.Clear();
}

void ONI_CALLBACK_TYPE Sensor::frameBackToPoolCallback(OniFrameInternal *pFrame,
                                                       void *pCookie) {
  // release the data
  if (pFrame->data != NULL) {
...
int Sensor::getDefaultRequiredFrameSize() {
  ...
  nRetVal = m_driverHandler.streamGetProperty(
      m_streamHandle, ONI_STREAM_PROPERTY_VIDEO_MODE, &videoMode, &size);
  XN_ASSERT(nRetVal == ONI_STATUS_OK);

  int stride;
  size = sizeof(stride);
  nRetVal = m_driverHandler.streamGetProperty(
      m_streamHandle, ONI_STREAM_PROPERTY_STRIDE, &stride, &size);
  if (nRetVal != ONI_STATUS_OK) {
    stride =
        videoMode.resolutionX * oniFormatBytesPerPixel(videoMode.pixelFormat);
  }

  return stride * videoMode.resolutionY;
}

void ONI_CALLBACK_TYPE Sensor::newFrameCallback(void * /*streamHandle*/,
                                                OniFrame *pFrame,
                                                void *pCookie) {
  Sensor *pThis = (Sensor *)pCookie;
...
int ONI_CALLBACK_TYPE
Sensor::getDefaultRequiredFrameSizeCallback(void *streamServices) {
...
void ONI_CALLBACK_TYPE Sensor::addFrameRefCallback(void *streamServices,
                                                   OniFrame *pFrame) {
  Sensor *pThis = (Sensor *)streamServices;
  return pThis->m_frameManager.addRef(pFrame);
}

void ONI_CALLBACK_TYPE Sensor::releaseFrameCallback(void *streamServices,
                                                    OniFrame *pFrame) {
  Sensor *pThis = (Sensor *)streamServices;

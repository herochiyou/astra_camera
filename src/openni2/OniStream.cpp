...
VideoStream::VideoStream(Sensor *pSensor, const OniSensorInfo *pSensorInfo,
                         Device &device, const DriverHandler &libraryHandler,
                         FrameManager &frameManager,
                         xnl::ErrorLogger &errorLogger)
    : m_errorLogger(errorLogger), m_pSensorInfo(NULL), m_running(true),
      m_device(device), m_driverHandler(libraryHandler),
      m_frameManager(frameManager), m_pSensor(pSensor), m_hNewFrameEvent(NULL),
      m_started(FALSE) {
  ...
  m_pSensorInfo->pSupportedVideoModes =
      XN_NEW_ARR(OniVideoMode, m_pSensorInfo->numSupportedVideoModes);
  xnOSMemCopy(m_pSensorInfo->pSupportedVideoModes,
              pSensorInfo->pSupportedVideoModes,
              sizeof(OniVideoMode) * m_pSensorInfo->numSupportedVideoModes);

  m_pSensor->newFrameEvent().Register(stream_NewFrame, this, m_hNewFrameEvent);

  m_driverHandler.streamSetPropertyChangedCallback(
      m_pSensor->streamHandle(), stream_PropertyChanged, this);
...
VideoStream::~VideoStream() {
  ...
  // Try to close the thread properly, and forcibly terminate it if
  // failed/timedout.
  m_running = false;
  xnOSSetEvent(m_newFrameInternalEvent);
  xnOSSetEvent(m_newFrameInternalEventForFrameHolder);
  XnStatus rc =
      xnOSWaitForThreadExit(m_newFrameThread, STREAM_DESTROY_THREAD_TIMEOUT);
  if (rc != XN_STATUS_OK) {
  ...
  if (m_device.getHandle() != NULL) {
    ...
    if (--m_pSensor->m_streamCount == 0) {
      m_driverHandler.deviceDestroyStream(m_device.getHandle(),
                                          m_pSensor->streamHandle());
    }
  }
...
OniBool VideoStream::isStarted() { return m_started; }

OniFrame *VideoStream::peekFrame() { return m_pFrameHolder->peekFrame(this); }

void VideoStream::lockFrame() { m_pFrameHolder->lock(); }

void VideoStream::unlockFrame() { m_pFrameHolder->unlock(); }

OniStatus VideoStream::setProperty(int propertyId, const void *data,
                                   int dataSize) {
  xnl::AutoCSLocker lock(m_pSensor->m_refCountCS);
  // if this stream is open, and not just by me (multiple depth streams for
  // example), don't allow any changes
  int myOpenRefCount = m_started ? 1 : 0;
  if (m_pSensor->m_startedStreamCount > myOpenRefCount) {
    m_errorLogger.Append("This stream is open by other components. "
                         "Configuration cannot be changed.");
    return ONI_STATUS_OUT_OF_FLOW;
  }

  OniStatus rc = m_driverHandler.streamSetProperty(m_pSensor->streamHandle(),
                                                   propertyId, data, dataSize);
  if (rc != ONI_STATUS_OK) {
...
OniStatus VideoStream::getProperty(int propertyId, void *data, int *pDataSize) {
  OniStatus rc = m_driverHandler.streamGetProperty(m_pSensor->streamHandle(),
                                                   propertyId, data, pDataSize);
  if (rc != ONI_STATUS_OK) {
...
OniBool VideoStream::isPropertySupported(int propertyId) {
  return m_driverHandler.streamIsPropertySupported(m_pSensor->streamHandle(),
                                                   propertyId);
}
...
OniStatus VideoStream::invoke(int commandId, void *data, int dataSize) {
  return m_driverHandler.streamInvoke(m_pSensor->streamHandle(), commandId,
                                      data, dataSize);
}
OniBool VideoStream::isCommandSupported(int commandId) {
  return m_driverHandler.streamIsCommandSupported(m_pSensor->streamHandle(),
                                                  commandId);
}
...
OniStatus VideoStream::registerNewFrameCallback(OniGeneralCallback handler,
                                                void *pCookie,
                                                XnCallbackHandle *pHandle) {
  return OniStatusFromXnStatus(
      m_newFrameEvent.Register(handler, pCookie, *pHandle));
}
...
OniStatus
VideoStream::setFrameBufferAllocator(OniFrameAllocBufferCallback alloc,
...
XN_THREAD_PROC VideoStream::newFrameThread(XN_THREAD_PARAM pThreadParam) {
  oni::implementation::VideoStream *pStream =
      (oni::implementation::VideoStream *)pThreadParam;
  pStream->newFrameThreadMainloop();

  XN_THREAD_PROC_RETURN(XN_STATUS_OK);
}

void ONI_CALLBACK_TYPE VideoStream::stream_NewFrame(OniFrame *pFrame,
                                                    void *pCookie) {
  // Validate parameters.
  ...
  // ignore frames if not started (this can happen if multiple streams exist for
  // the same sensor).
...

  {
    ...
    for (Recorders::Iterator i = pStream->m_recorders.Begin(),
...
Device &VideoStream::getDevice() { return m_device; }

void *VideoStream::getHandle() const { return m_pSensor->streamHandle(); }

void VideoStream::setFrameHolder(FrameHolder *pFrameHolder) {
  m_pFrameHolder = pFrameHolder;
}

FrameHolder *VideoStream::getFrameHolder() { return m_pFrameHolder; }

void ONI_CALLBACK_TYPE VideoStream::stream_PropertyChanged(
    void * /*streamHandle*/, int propertyId, const void *data, int dataSize,
    void *pCookie) {
  VideoStream *pStream = (VideoStream *)pCookie;
  if (NULL == pStream) {
    return;
  ...
  for (VideoStream::Recorders::Iterator i = pStream->m_recorders.Begin(),
                                        e = pStream->m_recorders.End();
       i != e; ++i) {
...
OniStatus
VideoStream::convertDepthToWorldCoordinates(float depthX, float depthY,
                                            float depthZ, float *pWorldX,
                                            float *pWorldY, float *pWorldZ) {
  if (m_pSensorInfo->sensorType != ONI_SENSOR_DEPTH) {
    m_errorLogger.Append(
        "convertDepthToWorldCoordinates: Stream is not from DEPTH\n");
...
OniStatus
VideoStream::convertWorldToDepthCoordinates(float worldX, float worldY,
                                            float worldZ, float *pDepthX,
                                            float *pDepthY, float *pDepthZ) {
  if (m_pSensorInfo->sensorType != ONI_SENSOR_DEPTH) {
    m_errorLogger.Append(
        "convertWorldToDepthCoordinates: Stream is not from DEPTH\n");
    return ONI_STATUS_NOT_SUPPORTED;
  }

  *pDepthX = m_worldConvertCache.coeffX * worldX / worldZ +
             m_worldConvertCache.halfResX;
  *pDepthY = m_worldConvertCache.halfResY -
             m_worldConvertCache.coeffY * worldY / worldZ;
  *pDepthZ = worldZ;
  return ONI_STATUS_OK;
}
...
void VideoStream::refreshWorldConversionCache() {
  ...
  m_worldConvertCache.coeffX =
      m_worldConvertCache.resolutionX / m_worldConvertCache.xzFactor;
  m_worldConvertCache.coeffY =
      m_worldConvertCache.resolutionY / m_worldConvertCache.yzFactor;
}

OniStatus VideoStream::convertDepthToColorCoordinates(VideoStream *colorStream,
                                                      int depthX, int depthY,
                                                      OniDepthPixel depthZ,
                                                      int *pColorX,
                                                      int *pColorY) {
  if (m_pSensorInfo->sensorType != ONI_SENSOR_DEPTH ||
      colorStream->m_pSensorInfo->sensorType != ONI_SENSOR_COLOR) {
    m_errorLogger.Append("convertDepthToColorCoordinates: Streams are from the "
                         "wrong sensors (should be DEPTH and COLOR)\n");
    return ONI_STATUS_NOT_SUPPORTED;
  }

  if (&m_device != &colorStream->m_device) {
    m_errorLogger.Append("convertDepthToColorCoordinates: Streams are not from "
                         "the same device\n");
    return ONI_STATUS_NOT_SUPPORTED;
  }
  return m_driverHandler.convertDepthPointToColor(
      m_pSensor->streamHandle(), colorStream->m_pSensor->streamHandle(), depthX,

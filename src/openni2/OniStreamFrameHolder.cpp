...
StreamFrameHolder::StreamFrameHolder(FrameManager &frameManager,
                                     VideoStream *pStream)
    : FrameHolder(frameManager), m_pStream(pStream), m_pLastFrame(NULL) {}

StreamFrameHolder::~StreamFrameHolder() { clear(); }

OniStatus StreamFrameHolder::readFrame(VideoStream *pStream,
                                       OniFrame **pFrame) {
...
OniStatus StreamFrameHolder::processNewFrame(VideoStream *pStream,
                                             OniFrame *pFrame) {
  // Verify called with relevant stream.
  if (pStream != m_pStream) {
    return ONI_STATUS_BAD_PARAMETER;
...
int StreamFrameHolder::getNumStreams() { return 1; }

void StreamFrameHolder::setStreamEnabled(VideoStream *pStream,
                                         OniBool enabled) {
  if (pStream != m_pStream)
    return;

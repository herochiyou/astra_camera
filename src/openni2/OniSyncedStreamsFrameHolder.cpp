...
// Constructor.
SyncedStreamsFrameHolder::SyncedStreamsFrameHolder(FrameManager &frameManager,
                                                   VideoStream **ppStreams,
                                                   int numStreams)
...
// Destructor.
SyncedStreamsFrameHolder::~SyncedStreamsFrameHolder() { clear(); }

// Get the next frame belonging to a stream.
OniStatus SyncedStreamsFrameHolder::readFrame(VideoStream *pStream,
                                              OniFrame **pFrame) {
  ...
  int frameId = (m_FrameSyncedStreams[0].pLastFrame != NULL)
                    ? m_FrameSyncedStreams[0].pLastFrame->frameIndex
                    : -1;
  ...
  for (XnUInt32 i = 0; i < numFrameSyncStreams; ++i) {
    ...
    if (m_FrameSyncedStreams[i].pStream == pStream) {
      ...
      if ((m_FrameSyncedStreams[i].pSyncedFrame == NULL) &&
        ...
        // NOTE: this is not a real recursion as next call will surely succeed
        // (new frame event received).
      ...
      } else // if (m_FameSyncedStreams[i].pLastFrame != NULL)
      {
        ...
        *pFrame = m_FrameSyncedStreams[i].pLastFrame;
        ...
        // Increment valid frame count, to make sure that after invalidation
        // last frames may still be copied to synced.
        ++validFrameCount;
      }
    }
    // Check if other synced frames exist.
    else if (m_FrameSyncedStreams[i].pSyncedFrame != NULL) {
      if ((minSyncedFrameId == -1) ||
          (minSyncedFrameId <
           m_FrameSyncedStreams[i].pSyncedFrame->frameIndex)) {
  ...
  // Check if there is are synced frames which have lower ID from the the frame
  // being returned.
...
OniStatus SyncedStreamsFrameHolder::processNewFrame(VideoStream *pStream,
                                                    OniFrame *pFrame) {
  // Make sure frame holder is enabled.
  if (!m_enabled) {
    return ONI_STATUS_OK;
  ...
  // Check if all the streams have valid frame, but no synced ones or no synced
  // frames retrieved.
  if (((syncedFramesCount == 0) ||
       (syncedFramesCount == numFrameSyncStreams)) &&
...
void SyncedStreamsFrameHolder::clear() {
  ...
  // Release all the streams from the frame sync group and release the stored
  // frames.
  XnUInt32 numFrameSyncStreams = m_FrameSyncedStreams.GetSize();
  for (XnUInt32 i = 0; i < numFrameSyncStreams; ++i) {
    if (m_FrameSyncedStreams[i].pLastFrame != NULL) {
...
void SyncedStreamsFrameHolder::setStreamEnabled(VideoStream *pStream,
                                                OniBool enabled) {
  lock();
  ...
  // NOTE: do NOT change m_enabled flag for now, to make sure no frames are
  // received.
...
void SyncedStreamsFrameHolder::getStreams(VideoStream **ppStreams,
                                          int *pNumStreams) {
  lock();
  ...
  *pNumStreams =
      (*pNumStreams > numFrameSyncStreams) ? numFrameSyncStreams : *pNumStreams;

...
ONI_C_API OniStatus oniRegisterDeviceCallbacks(OniDeviceCallbacks *pCallbacks,
  ...
  g_Context.registerDeviceConnectedCallback(
      pCallbacks->deviceConnected, pCookie,
      pDeviceHandles->deviceConnectedHandle);
  g_Context.registerDeviceDisconnectedCallback(
      pCallbacks->deviceDisconnected, pCookie,
      pDeviceHandles->deviceDisconnectedHandle);
  g_Context.registerDeviceStateChangedCallback(
      pCallbacks->deviceStateChanged, pCookie,
      pDeviceHandles->deviceStateChangedHandle);
...
ONI_C_API void oniUnregisterDeviceCallbacks(OniCallbackHandle handle) {
  ...
  g_Context.unregisterDeviceConnectedCallback(
      pDevicesHandles->deviceConnectedHandle);
  g_Context.unregisterDeviceDisconnectedCallback(
      pDevicesHandles->deviceDisconnectedHandle);
  g_Context.unregisterDeviceStateChangedCallback(
      pDevicesHandles->deviceStateChangedHandle);

  XN_DELETE(pDevicesHandles);
}

ONI_C_API OniStatus oniWaitForAnyStream(OniStreamHandle *pStreams,
                                        int streamCount, int *pStreamIndex,
                                        int timeout) {
  g_Context.clearErrorLogger();
...
ONI_C_API OniStatus oniDeviceOpenEx(const char *uri, const char *mode,
...
ONI_C_API const OniSensorInfo *
oniDeviceGetSensorInfo(OniDeviceHandle device, OniSensorType sensorType) {
  g_Context.clearErrorLogger();
...
ONI_C_API OniStatus oniDeviceSetProperty(OniDeviceHandle device, int propertyId,
                                         const void *data, int dataSize) {
  g_Context.clearErrorLogger();
  return device->pDevice->setProperty(propertyId, data, dataSize);
}
ONI_C_API OniStatus oniDeviceGetProperty(OniDeviceHandle device, int propertyId,
...
ONI_C_API OniStatus oniDeviceInvoke(OniDeviceHandle device, int commandId,
...
ONI_C_API OniBool oniDeviceIsImageRegistrationModeSupported(
    OniDeviceHandle device, OniImageRegistrationMode mode) {
  g_Context.clearErrorLogger();
...
void ONI_CALLBACK_TYPE OniNewFrameTranslationHandler(void *pCookie) {
  ...
  (*pNewFrameCookie->handler)(pNewFrameCookie->streamHandle,
                              pNewFrameCookie->pCookie);
}

ONI_C_API OniStatus oniStreamRegisterNewFrameCallback(
    OniStreamHandle stream, OniNewFrameCallback handler, void *pCookie,
    OniCallbackHandle *pHandle) {
  g_Context.clearErrorLogger();

  if (*pHandle != NULL) {
    // Already registered to something
    g_Context.addToLogger(
        "Can't register same listener instance to multiple events");
    return ONI_STATUS_ERROR;
  }
  ...
  *pHandle = (OniCallbackHandle)pNewFrameCookie;
  return stream->pStream->registerNewFrameCallback(
      OniNewFrameTranslationHandler, pNewFrameCookie,
      &(pNewFrameCookie->handle));
}
...
ONI_C_API OniStatus oniStreamSetProperty(OniStreamHandle stream, int propertyId,
                                         const void *data, int dataSize) {
  g_Context.clearErrorLogger();
  return stream->pStream->setProperty(propertyId, data, dataSize);
}

ONI_C_API OniStatus oniStreamGetProperty(OniStreamHandle stream, int propertyId,
                                         void *data, int *pDataSize) {
  g_Context.clearErrorLogger();
...
ONI_C_API OniStatus oniStreamInvoke(OniStreamHandle stream, int commandId,
                                    void *data, int dataSize) {
  g_Context.clearErrorLogger();
...
ONI_C_API OniStatus oniStreamSetFrameBuffersAllocator(
    OniStreamHandle stream, OniFrameAllocBufferCallback alloc,
    OniFrameFreeBufferCallback free, void *pCookie) {
  g_Context.clearErrorLogger();
...
ONI_C_API OniStatus oniRecorderAttachStream(OniRecorderHandle recorder,
  ...
  // Validate parameters.
  if (NULL == recorder || NULL == recorder->pRecorder || NULL == stream ||
      NULL == stream->pStream) {
    return ONI_STATUS_BAD_PARAMETER;
  }
  // Attach a stream to the recorder.
  return recorder->pRecorder->attachStream(*stream->pStream,
...
ONI_C_API void oniWriteLogEntry(const char *mask, int severity,
                                const char *message) {
...
ONI_C_API OniStatus oniSetLogMinSeverity(int nMinSeverity) {
  XnStatus rc =
      xnLogSetMaskMinSeverity(XN_LOG_MASK_ALL, (XnLogSeverity)nMinSeverity);
...
#endif
ONI_C_API OniStatus oniCoordinateConverterDepthToWorld(
    OniStreamHandle depthStream, float depthX, float depthY, float depthZ,
    float *pWorldX, float *pWorldY, float *pWorldZ) {
  g_Context.clearErrorLogger();
  return depthStream->pStream->convertDepthToWorldCoordinates(
      depthX, depthY, depthZ, pWorldX, pWorldY, pWorldZ);
}

ONI_C_API OniStatus oniCoordinateConverterWorldToDepth(
    OniStreamHandle depthStream, float worldX, float worldY, float worldZ,
    float *pDepthX, float *pDepthY, float *pDepthZ) {
  g_Context.clearErrorLogger();
  return depthStream->pStream->convertWorldToDepthCoordinates(
      worldX, worldY, worldZ, pDepthX, pDepthY, pDepthZ);
}

ONI_C_API OniStatus oniCoordinateConverterDepthToColor(
    OniStreamHandle depthStream, OniStreamHandle colorStream, int depthX,
    int depthY, OniDepthPixel depthZ, int *pColorX, int *pColorY) {
  g_Context.clearErrorLogger();
  return depthStream->pStream->convertDepthToColorCoordinates(
      colorStream->pStream, depthX, depthY, depthZ, pColorX, pColorY);
}

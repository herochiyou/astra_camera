...
enum {
  ...
  XN_MODULE_PROPERTY_RESET_SENSOR_ON_STARTUP =
      0x1080F004, // "ResetSensorOnStartup"
  /** unsigned long long, get only */
  ...
  XN_MODULE_PROPERTY_CLOSE_STREAMS_ON_SHUTDOWN =
      0x1080FF78, // "CloseStreamsOnShutdown"
  /** Integer */
  XN_MODULE_PROPERTY_FIRMWARE_LOG_INTERVAL =
      0x1080FF7F, // "FirmwareLogInterval"
  /** Boolean */
  ...
  XN_MODULE_PROPERTY_FIRMWARE_CPU_INTERVAL =
      0x1080FF83, // "FirmwareCPUInterval"
  /** String, get only */
  ...
  XN_MODULE_PROPERTY_SENSOR_PLATFORM_STRING =
      0x1080FF7C, // "SensorPlatformString"
  ...
  XN_MODULE_PROPERTY_TEC_FAST_CONVERGENCE_STATUS =
      0x1080FF8B, // "TecFastConvergenceStatus"
  ...
  XN_MODULE_PROPERTY_IS_SUPPORT_LASER_SECURE =
      0x1080FF9B, // is support laser secure
  XN_MODULE_PROPERTY_LASER_SECURE_STATUS =
      0x1080FF9C, // laser secure (open or close)
  XN_MODULE_PROPERTY_LASER_SECURE_KEEPALIVE =
      0x1080FF9D, // keepalive heart packet
  // laser current
  ...
  // IR real time Temperature mode,If it is binocular, it indicates the left IR
  // temperature.
  ...
  XN_MODULE_PROPERTY_PRODUCT_SERIAL_NUMBER =
      0x1080FFC8, // product write sn(S1/T1)
  XN_MODULE_PROPERTY_CORE_BROAD_FLASH_ID =
      0x1080FFC9, // mipi project core broad status
  // ir flood level
  ...
  XN_STREAM_PROPERTY_WHITE_BALANCE_ENABLED =
      0x10801002, // "WhiteBalancedEnabled"
  /** unsigned long long */
  ...
  XN_STREAM_PROPERTY_WAVELENGTH_CORRECTION =
      0x1080FF46, // "WavelengthCorrection"
  /** Boolean */
  XN_STREAM_PROPERTY_WAVELENGTH_CORRECTION_DEBUG =
      0x1080FF47, // "WavelengthCorrectionDebug"

  // DCW2
  XN_MODULE_PROPERTY_LDP_CALIBRATION_BOOL =
      0x1080FF70, //"LDP calibration status"
  ...
  XN_STREAM_PROPERTY_SOFTWARE_REGISTRATION =
      0x2080FF42,                                  // "Software Registration"
  ...
  XN_STREAM_PROPERTY_DEPTH_MAX_SPECKLE_SIZE =
      0x2080FF46, // soft filter maxSpeckleSize
  XN_STREAM_PROPERTY_DEPTH_UNIT_COEFFICIENT =
      0x2080FF47, // Calibration unit accuracy coefficient
  XN_STREAM_PROPERTY_DEPTH_LEFT_EXTEND =
      0x2080FF48, // Calibration unit accuracy coefficient
  XN_STREAM_PROPERTY_DEPTH_ORIGINAL_SHIFT =
      0x2080FF49, // original shift or openni shift
  XN_STREAM_PROPERTY_DEPTH_SHIFT_COMPENSATION =
      0x2080FF4A, // shift compensation
...
typedef enum {
  ...
  // New addition
  XN_IO_DEPTH_FORMAT_UNCOMPRESSED_16_BIT_DEPTH =
      5, // No compression 16 bit depth, no parallax depth
  ...
  XN_IO_DEPTH_FORMAT_COMPRESSED_16_BIT_DEPTH =
      10, // Compress 16bit depth without parallax depth

  // Standard Mipi packing raw data unpacking type is supported,
  // non-standard Mipi packing data unpacking is not supported
  XN_IO_DEPTH_FORMAT_UNCOMPRESSED_10_BIT_MIPI =
      11, // uncompressed 10bit Mipi package parallax
  XN_IO_DEPTH_FORMAT_UNCOMPRESSED_11_BIT_MIPI =
      12, // uncompressed 11bit Mipi package parallax
  XN_IO_DEPTH_FORMAT_UNCOMPRESSED_12_BIT_MIPI =
      13, // uncompressed 12bit Mipi package parallax
...
typedef enum { XnFileAttributeReadOnly = 0x8000 } XnFilePossibleAttributes;
...
typedef enum XnBistType {
  ...
  XN_BIST_ALL = (0xFFFFFFFF & ~XN_BIST_NESA_TEST_MASK &
...
typedef struct XnEmitterData {
  ...
  uint16_t
      m_EmitterSet; // duty cycle on emitter set in system clocks (high time).
  uint16_t
      m_EmitterSettingLogic;    // TRUE = positive logic, FALSE = negative logic
...
typedef enum {
  PROTOCOL_DEPTH_NIR =
      0, // Depth + pure IR interleaving (USB device default mode)
...
enum THREAD_UPDATE_STATE {
  ...
  THREAD_UPDATE_STATE_SAVE_ROM_FILE_FAILED =
      -2, // upgrade write save ROM file failed
  THREAD_UPDATE_STATE_SAVE_WRITE_END_MARK_FILE_FAILED =
      -3, // upgrade write save end flag file failed
};

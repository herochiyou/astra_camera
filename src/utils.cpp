...
bool operator==(const openni::VideoMode &lhs, const openni::VideoMode &rhs) {
  return lhs.getResolutionY() == rhs.getResolutionY() &&
         lhs.getResolutionX() == rhs.getResolutionX() &&
         lhs.getFps() == rhs.getFps() &&
...
std::ostream &operator<<(std::ostream &os,
                         const openni::VideoMode &video_mode) {
  os << "Resolution :" << video_mode.getResolutionX() << "x"
     << video_mode.getResolutionY() << "@" << video_mode.getFps() << "Hz"
     << std::endl
...
tf2::Quaternion rotationMatrixToQuaternion(const std::vector<float> &rotation) {
  ...
  m << rotation[0], rotation[3], rotation[6], rotation[1], rotation[4],
...
void saveRGBPointToPly(sensor_msgs::PointCloud2::Ptr cloud,
  ...
  for (size_t i = 0; i < point_size; ++i) {
    fprintf(fp, "%.3f %.3f %.3f %d %d %d\n", points[i].x, points[i].y,
            points[i].z, rgb[i][0], rgb[i][1], rgb[i][2]);
  }
  fflush(fp);
  fclose(fp);
}

...
#if (XN_PLATFORM == XN_PLATFORM_LINUX_X86 ||                                   \
     XN_PLATFORM == XN_PLATFORM_LINUX_ARM)
#include <libudev.h>
#define XN_USE_UDEV
#endif
...
void xnUSBDeviceConnected(struct udev_device *dev) {
  ...
  pConnected->nVendorID =
      strtoul(udev_device_get_sysattr_value(dev, "idVendor"), NULL, 16);
  pConnected->nProductID =
      strtoul(udev_device_get_sysattr_value(dev, "idProduct"), NULL, 16);
  pConnected->nBusNum =
      strtoul(udev_device_get_sysattr_value(dev, "busnum"), NULL, 10);
  pConnected->nDevNum =
      strtoul(udev_device_get_sysattr_value(dev, "devnum"), NULL, 10);

  // copy the device node path aside, to be used upon removal
  xnOSStrCopy(pConnected->strNode, udev_device_get_devnode(dev),
              XN_FILE_MAX_PATH);

  // generate our unique URI
  snprintf(pConnected->strDevicePath, XN_FILE_MAX_PATH, "%04hx/%04hx@%hhu/%hhu",
           pConnected->nVendorID, pConnected->nProductID, pConnected->nBusNum,
           pConnected->nDevNum);
  ...
  for (XnUSBEventCallbackList::Iterator it = g_connectivityEvent.Begin();
       it != g_connectivityEvent.End(); ++it) {
    XnUSBEventCallback *pCallback = *it;

    if (pCallback->nVendorID == pConnected->nVendorID &&
        pCallback->nProductID == pConnected->nProductID) {
      XnUSBEventArgs args;
...
void xnUSBDeviceDisconnected(struct udev_device *dev) {
  ...
  for (XnUSBConnectedDeviceList::Iterator it = g_connectedDevices.Begin();
       it != g_connectedDevices.End(); ++it) {
    if (!xnOSStrCmp(((XnUSBConnectedDevice *)*it)->strNode,
  ...
  if (!pConnected) {
    ...
    xnLogWarning(XN_MASK_USB,
                 "Got device disconnection event - for an unknown device!");
    return;
  }
    ...
    if (ret > 0 && FD_ISSET(fd, &fds)) {
      ...
      if (dev) {
        ...
        */
        ...
        xnLogWarning(
            XN_MASK_USB,
            "No Device from udev_monitor_receive_device(). An error occured.");
      }
    }
  }
...
XnStatus xnUSBAsynchThreadAddRef() {
  ...
  if (g_InitData.hThread == NULL) {
    ...
    // and start thread
    nRetVal =
        xnOSCreateThread(xnUSBHandleEventsThread, NULL, &g_InitData.hThread);
    if (nRetVal != XN_STATUS_OK) {
      // clean-up
      xnUSBPlatformSpecificShutdown();
      return nRetVal;
    ...
    if (nRetVal != 0) {
      xnLogWarning(XN_MASK_USB,
                   "USB events thread: Failed to set thread priority to "
                   "critical. This might cause loss of data...");
      printf("Warning: USB events thread - failed to set priority. This might "
             "cause loss of data...\n");
    }
  }
...
void xnUSBAsynchThreadStop() {
  if (g_InitData.hThread != NULL) {
    ...
    XnStatus nRetVal = xnOSWaitForThreadExit(g_InitData.hThread,
                                             XN_USB_HANDLE_EVENTS_TIMEOUT * 2);
    if (nRetVal != XN_STATUS_OK) {
      xnLogWarning(XN_MASK_USB,
                   "USB events thread didn't shutdown. Terminating it...");
...
/*
 * Finds a USB device.
 * the returned device must be unreferenced when it is no longer needed using
 * libusb_unref_device.
 */
XnStatus FindDevice(XnUInt16 nVendorID, XnUInt16 nProductID, void *pExtraParam,
                    libusb_device **ppDevice) {
  *ppDevice = NULL;

  // get device list
  libusb_device **ppDevices;
  ssize_t nDeviceCount =
      libusb_get_device_list(g_InitData.pContext, &ppDevices);
  ...
  for (ssize_t i = 0; i < nDeviceCount; ++i) {
    ...
    if (desc.idVendor == nVendorID && desc.idProduct == nProductID) {
      // add a reference to the device (so it won't be destroyed when list is
      // freed)
...
XN_C_API XnStatus xnUSBIsDevicePresent(XnUInt16 nVendorID, XnUInt16 nProductID,
                                       void *pExtraParam,
...
XN_C_API XnStatus xnUSBEnumerateDevices(
    XnUInt16 nVendorID, XnUInt16 nProductID,
    const XnUSBConnectionString **pastrDevicePaths, XnUInt32 *pnCount) {
  // get device list
  libusb_device **ppDevices;
  ssize_t nDeviceCount =
      libusb_get_device_list(g_InitData.pContext, &ppDevices);
  ...
  // allocate array
  XnUSBConnectionString *aResult = (XnUSBConnectionString *)xnOSCalloc(
      nCount, sizeof(XnUSBConnectionString));
  if (aResult == NULL) {
    libusb_free_device_list(ppDevices, 1);
  ...
  for (ssize_t i = 0; i < nDeviceCount; ++i) {
    ...
    // check if this is the requested device
    if (desc.idVendor == nVendorID && desc.idProduct == nProductID) {
      sprintf(aResult[nCurrent], "%04hx/%04hx@%hhu/%hhu", nVendorID, nProductID,
              libusb_get_bus_number(pDevice),
...
XN_C_API void
xnUSBFreeDevicesList(const XnUSBConnectionString *astrDevicePaths) {
  xnOSFree(astrDevicePaths);
}

XN_C_API XnStatus xnUSBOpenDeviceImpl(libusb_device *pDevice,
  ...
  /*
          // set for the first (and only) configuration (this will perform a
     light-weight reset) rc = libusb_set_configuration(handle, 1); if (rc != 0)
          {
                  libusb_close(handle);
                  return (XN_STATUS_USB_SET_CONFIG_FAILED);
          }
  */
  // claim the interface (you cannot open any end point before claiming the
  // interface)
...
XN_C_API XnStatus xnUSBOpenDevice(XnUInt16 nVendorID, XnUInt16 nProductID,
                                  void *pExtraParam, void *pExtraParam2,
                                  XN_USB_DEV_HANDLE *pDevHandlePtr) {
  XnStatus nRetVal = XN_STATUS_OK;
...
XN_C_API XnStatus
xnUSBOpenDeviceByPath(const XnUSBConnectionString strDevicePath,
                      XN_USB_DEV_HANDLE *pDevHandlePtr) {
  XnStatus nRetVal = XN_STATUS_OK;
  ...
  XnUInt8 nBus = 0;
  XnUInt8 nAddress = 0;
  sscanf(strDevicePath, "%hx/%hx@%hhu/%hhu", &nVendorID, &nProductID, &nBus,
         &nAddress);

  if (nVendorID == 0 || nProductID == 0 || nBus == 0 || nAddress == 0) {
    XN_LOG_WARNING_RETURN(XN_STATUS_USB_DEVICE_OPEN_FAILED,
                          "Invalid connection string: %s", strDevicePath);
  }
  ...
  for (ssize_t i = 0; i < nDeviceCount; ++i) {
    ...
    // check if this is the requested device
    if (desc.idVendor == nVendorID && desc.idProduct == nProductID &&
        libusb_get_bus_number(pDevice) == nBus &&
        libusb_get_device_address(pDevice) == nAddress) {
      // add a reference to the device (so it won't be destroyed when list is
      // freed)
      libusb_ref_device(pDevice);
...
XN_C_API XnStatus xnUSBCloseDevice(XN_USB_DEV_HANDLE pDevHandle) {
  ...
  int rc =
      libusb_release_interface(pDevHandle->hDevice, pDevHandle->nInterface);
  if (0 != rc) {
...
XN_C_API XnStatus xnUSBSetInterface(XN_USB_DEV_HANDLE pDevHandle,
                                    XnUInt8 nInterface, XnUInt8 nAltInterface) {
  // validate parameters
  XN_VALIDATE_USB_INIT();
  XN_VALIDATE_DEVICE_HANDLE(pDevHandle);

  int rc = libusb_set_interface_alt_setting(pDevHandle->hDevice, nInterface,
                                            nAltInterface);
  if (rc != 0) {
    return (XN_STATUS_USB_SET_INTERFACE_FAILED);
  }
...
XN_C_API XnStatus xnUSBOpenEndPoint(XN_USB_DEV_HANDLE pDevHandle,
  ...
  // take that interface
  const libusb_interface *pInterface =
      &pConfig->interface[pDevHandle->nInterface];
  ...
  const libusb_interface_descriptor *pInterfaceDesc =
      &pInterface->altsetting[pDevHandle->nAltSetting];
  ...
  libusb_transfer_type transfer_type =
      (libusb_transfer_type)(pEndpointDesc->bmAttributes & 0x3); // lower 2-bits

  // calculate max packet size
  // NOTE: we do not use libusb functions
  // (libusb_get_max_packet_size/libusb_get_max_iso_packet_size) because they
  // hace a bug and does not consider alternative interface
  ...
  if (transfer_type == LIBUSB_TRANSFER_TYPE_ISOCHRONOUS) {
    ...
    // bits 11 and 12 mark the number of additional transactions, bits 0-10 mark
    // the size
  ...
  libusb_endpoint_direction direction =
      (libusb_endpoint_direction)(nEndPointID & 0x80); // 8th bit
  ...
  // allocate handle
  XN_VALIDATE_ALIGNED_CALLOC(*pEPHandlePtr, XnUSBEPHandle, 1,
...
XN_C_API XnStatus xnUSBSendControl(XN_USB_DEV_HANDLE pDevHandle,
                                   XnUSBControlType nType, XnUInt8 nRequest,
                                   XnUInt16 nValue, XnUInt16 nIndex,
                                   XnUChar *pBuffer, XnUInt32 nBufferSize,
                                   XnUInt32 nTimeOut) {
  // validate parameters
  XN_VALIDATE_USB_INIT();
  XN_VALIDATE_DEVICE_HANDLE(pDevHandle);
  ...
  // send
  int nBytesSent =
      libusb_control_transfer(pDevHandle->hDevice, bmRequestType, nRequest,
                              nValue, nIndex, pBuffer, nBufferSize, nTimeOut);
...
XN_C_API XnStatus xnUSBReceiveControl(XN_USB_DEV_HANDLE pDevHandle,
  ...
  *pnBytesReceived = 0;
  ...
  {
    xnLogWarning(XN_MASK_USB,
                 "Failed to receive from USB control endpoint (%d)",
                 nBytesReceived);
...
XN_C_API XnStatus xnUSBWriteEndPoint(XN_USB_EP_HANDLE pEPHandle,
  ...
  if (pEPHandle->nType == XN_USB_EP_BULK) {
    rc = libusb_bulk_transfer(pEPHandle->hDevice, pEPHandle->nAddress, pBuffer,
                              nBufferSize, &nBytesSent, nTimeOut);
  } else if (pEPHandle->nType == XN_USB_EP_INTERRUPT) {
    rc = libusb_interrupt_transfer(pEPHandle->hDevice, pEPHandle->nAddress,
                                   pBuffer, nBufferSize, &nBytesSent, nTimeOut);
  } else {
...
XN_C_API XnStatus xnUSBReadEndPoint(XN_USB_EP_HANDLE pEPHandle,
  ...
  *pnBytesReceived = 0;
  ...
  if (pEPHandle->nType == XN_USB_EP_BULK) {
    rc = libusb_bulk_transfer(pEPHandle->hDevice, pEPHandle->nAddress, pBuffer,
                              nBufferSize, &nBytesReceived, nTimeOut);
  } else if (pEPHandle->nType == XN_USB_EP_INTERRUPT) {
    rc = libusb_interrupt_transfer(pEPHandle->hDevice, pEPHandle->nAddress,
                                   pBuffer, nBufferSize, &nBytesReceived,
                                   nTimeOut);
  } else {
    return (XN_STATUS_USB_UNSUPPORTED_ENDPOINT_TYPE);
  }
...
XN_THREAD_PROC xnUSBReadThreadMain(XN_THREAD_PARAM pThreadParam) {
  ...
  // set thread priority to critical
  XnStatus nRetVal =
      xnOSSetThreadPriority(pThreadData->hReadThread, XN_PRIORITY_CRITICAL);
  if (nRetVal != 0) {
    xnLogWarning(XN_MASK_USB, "Failed to set thread priority to critical. This "
  ...
  for (XnUInt32 i = 0; i < pThreadData->nNumBuffers; ++i) {
    ...
    if (rc != 0) {
      xnLogError(XN_MASK_USB,
                 "Endpoint 0x%x, Buffer %d: Failed to submit asynch I/O "
                 "transfer (err=%d)!",
                 pTransfer->endpoint, pBufferInfo->nBufferID, rc);
    }
  }
  ...
  while (TRUE) {
    for (XnUInt32 i = 0; i < pThreadData->nNumBuffers; ++i) {
      // check if thread can exit (only after kill was requested, and all
      // transfers returned)
      ...
      nRetVal = xnOSWaitEvent(pBufferInfo->hEvent, pThreadData->bKillReadThread
                                                       ? 0
                                                       : pThreadData->nTimeOut);
      if (nRetVal == XN_STATUS_OS_EVENT_TIMEOUT) {
        //				xnLogWarning(XN_MASK_USB, "Endpoint
        // 0x%x, Buffer %d: timeout. cancelling transfer...",
        // pTransfer->endpoint, pBufferInfo->nBufferID);

        // cancel it
        int rc = libusb_cancel_transfer(pBufferInfo->transfer);
        if (rc != 0) {
          // If we get LIBUSB_ERROR_NOT_FOUND it means that the transfer was
          // already cancaled/completed which is a very common thing during
          // normal shutdown so there's no need to print it.
          if (rc != LIBUSB_ERROR_NOT_FOUND) {
            if (rc == LIBUSB_ERROR_NO_DEVICE) {
              goto disconnect;
            } else {
              xnLogError(XN_MASK_USB,
                         "Endpoint 0x%x, Buffer %d: Failed to cancel asynch "
                         "I/O transfer (err=%d)!",
                         pTransfer->endpoint, pBufferInfo->nBufferID, rc);
            }
          }
      ...
      if (nRetVal != XN_STATUS_OK) {
        // not much we can do
        xnLogWarning(XN_MASK_USB,
                     "Endpoint 0x%x, Buffer %d: Failed waiting on asynch "
                     "transfer event: %s",
                     pTransfer->endpoint, pBufferInfo->nBufferID,
      ...
      if (pBufferInfo->bIsQueued) {
        xnLogWarning(XN_MASK_USB,
                     "Endpoint 0x%x, Buffer %d: Transfer is still queued "
                     "though event was raised!",
                     pTransfer->endpoint, pBufferInfo->nBufferID);
        // TODO: cancel it?
      } else // transfer done
      {
        ...
        if (pBufferInfo->nLastStatus ==
                LIBUSB_TRANSFER_COMPLETED || // read succeeded
            pBufferInfo->nLastStatus ==
                LIBUSB_TRANSFER_CANCELLED) // cancelled, but maybe some data
                                           // arrived
        {
          if (pTransfer->type == LIBUSB_TRANSFER_TYPE_ISOCHRONOUS) {
            ...
            // some packets may return empty or partial, aggregate as many
            // consequent packets as possible, and then send them to processing
            for (XnInt32 i = 0; i < pTransfer->num_iso_packets; ++i) {
              struct libusb_iso_packet_descriptor *pPacket =
                  &pTransfer->iso_packet_desc[i];
              ...
              if (pPacket->status == LIBUSB_TRANSFER_COMPLETED) {
                ...
                if (pPacket->status !=
                    LIBUSB_TRANSFER_ERROR) // Skip printing uninformative common
                                           // errors
                {
                  xnLogWarning(XN_MASK_USB,
                               "Endpoint 0x%x, Buffer %d, packet %d Asynch "
                               "transfer failed (status: %d)",
                               pTransfer->endpoint, pBufferInfo->nBufferID, i,
                               pPacket->status);
                }
              ...
              {
                if (nTotalBytes != 0) {
                  pBufferInfo->pThreadData->pCallbackFunction(
                      pBuffer, nTotalBytes,
                      pBufferInfo->pThreadData->pCallbackData);
                }
                pBuffer = NULL;
                nTotalBytes = 0;
              }
            ...
            pBufferInfo->pThreadData->pCallbackFunction(
                pTransfer->buffer, pTransfer->actual_length,
                pBufferInfo->pThreadData->pCallbackData);
          }
          ...
          xnLogWarning(
              XN_MASK_USB,
              "Endpoint 0x%x, Buffer %d: Asynch transfer failed (status: %d)",
              pTransfer->endpoint, pBufferInfo->nBufferID, pTransfer->status);
        }
        ...
        if (!pBufferInfo->pThreadData->bKillReadThread) {
          ...
          if (rc != 0) {
            ...
            xnLogError(XN_MASK_USB,
                       "Endpoint 0x%x, Buffer %d: Failed to re-submit asynch "
                       "I/O transfer (err=%d)!",
                       pTransfer->endpoint, pBufferInfo->nBufferID, rc);
          }
        }
...
disconnect:
  xnLogError(XN_MASK_USB,
             "Unexpected device disconnect, aborting the read thread!");

  // close the thread, the device is gone...
  XN_THREAD_PROC_RETURN(XN_STATUS_OK);
}

/* This function is called whenever transfer is done (successfully or with an
 * error). */
void xnTransferCallback(libusb_transfer *pTransfer) {
  ...
  // keep the status (according to libusb documentation, this field is invalid
  // outside the callback method)
  ...
  if (nRetVal != XN_STATUS_OK) {
    ...
    xnLogWarning(XN_MASK_USB, "Failed to set event for buffer: %s",
                 xnGetStatusString(nRetVal));
  }
}

XN_C_API XnStatus xnUSBInitReadThread(
    XN_USB_EP_HANDLE pEPHandle, XnUInt32 nBufferSize, XnUInt32 nNumBuffers,
    XnUInt32 nTimeOut, XnUSBReadCallbackFunctionPtr pCallbackFunction,
    void *pCallbackData) {
  XnStatus nRetVal = XN_STATUS_OK;
  ...
  // allocate buffers
  pThreadData->pBuffersInfo = (XnUSBBuffersInfo *)xnOSCallocAligned(
      nNumBuffers, sizeof(XnUSBBuffersInfo), XN_DEFAULT_MEM_ALIGN);
  ...
  for (XnUInt32 i = 0; i < nNumBuffers; i++) {
    ...
    // allocate buffer
    XnUChar *pBuffer = (unsigned char *)xnOSCallocAligned(
        nBufferSize, sizeof(XnUChar), XN_DEFAULT_MEM_ALIGN);
    if (pBuffer == NULL) {
      xnCleanupThreadData(pThreadData);
      return (XN_STATUS_ALLOC_FAILED);
    }
    ...
    if (pEPHandle->nType == XN_USB_EP_BULK) {
      libusb_fill_bulk_transfer(pTransfer, pEPHandle->hDevice,
                                pEPHandle->nAddress, pBuffer, nBufferSize,
                                xnTransferCallback, pBufferInfo, 0);
    } else if (pEPHandle->nType == XN_USB_EP_INTERRUPT) {
      libusb_fill_interrupt_transfer(pTransfer, pEPHandle->hDevice,
                                     pEPHandle->nAddress, pBuffer, nBufferSize,
                                     xnTransferCallback, pBufferInfo, 0);
    } else if (pEPHandle->nType == XN_USB_EP_ISOCHRONOUS) {
      libusb_fill_iso_transfer(
          pTransfer, pEPHandle->hDevice, pEPHandle->nAddress, pBuffer,
          nBufferSize, nNumIsoPackets, xnTransferCallback, pBufferInfo, 0);
  ...
  nRetVal = xnOSCreateThread(xnUSBReadThreadMain, &pEPHandle->ThreadData,
                             &pThreadData->hReadThread);
  if (nRetVal != XN_STATUS_OK) {
    xnCleanupThreadData(pThreadData);
    return (nRetVal);
  }
...
XN_C_API XnStatus xnUSBShutdownReadThread(XN_USB_EP_HANDLE pEPHandle) {
  ...
  if (pThreadData->hReadThread != NULL) {
    ...
    // PATCH: we don't cancel the requests, because there is a bug causing
    // segmentation fault.
#if XN_PLATFORM == XN_PLATFORM_ANDROID_ARM
    ...
    for (XnUInt32 i = 0; i < pThreadData->nNumBuffers; ++i) {
      if (pThreadData->pBuffersInfo[i].bIsQueued) {
        ...
        // NOTE: we don't check error code. In any case we still need to wait
        // for all transfers to complete. If cancelling was succeeded, they will
        // return immediately. If not, they will reach timeout and return
      }
    }
#endif

    // now wait for thread to exit (we wait the timeout of all buffers + an
    // extra second)
    XnStatus nRetVal = xnOSWaitForThreadExit(
        pThreadData->hReadThread,
        pThreadData->nTimeOut * pThreadData->nNumBuffers + 1000);
    if (nRetVal != XN_STATUS_OK) {
...
XN_C_API XnStatus xnUSBSetCallbackHandler(
    XnUInt16 nVendorID, XnUInt16 /*nProductID*/, void *pExtraParam,
    XnUSBEventCallbackFunctionPtr pCallbackFunction, void *pCallbackData) {
  return (XN_STATUS_OS_UNSUPPORTED_FUNCTION);
}

XN_C_API XnStatus XN_C_DECL xnUSBRegisterToConnectivityEvents(
    XnUInt16 nVendorID, XnUInt16 nProductID,
    XnUSBDeviceCallbackFunctionPtr pFunc, void *pCookie,
...
XN_C_API void XN_C_DECL
xnUSBUnregisterFromConnectivityEvents(XnRegistrationHandle hRegistration) {
  XnUSBEventCallback *pCallback =
      reinterpret_cast<XnUSBEventCallback *>(hRegistration);

...
template <class TKey> class DefaultKeyManager {
  public:
  static xnl::HashCode Hash(const TKey &key) { return ((XnSizeT)key) & 0xff; }
...
class Hash {
  ...
  enum { LAST_BIN = (1 << sizeof(xnl::HashCode) * 8), NUM_BINS = LAST_BIN + 1 };
  class ConstIterator {
    public:
    ConstIterator() : m_ppBins(NULL), m_currBin(0) {}
    ConstIterator(TPairList *const *apBins, XnUInt32 currBin,
    ...
    ConstIterator(const ConstIterator &other)
        : m_ppBins(other.m_ppBins), m_currBin(other.m_currBin),
          m_currIt(other.m_currIt) {}
    ...
    ConstIterator &operator++() {
      ...
      if (m_currIt == m_ppBins[m_currBin]->End()) {
        do {
          ++m_currBin;
        } while (m_currBin < LAST_BIN && (m_ppBins[m_currBin] == NULL ||
                                          m_ppBins[m_currBin]->IsEmpty()));
    ...
    inline const TPair &operator*() const { return *m_currIt; }
    inline const TPair *operator->() const { return m_currIt.operator->(); }
  ...
  class Iterator : public ConstIterator {
    ...
    Iterator(TPairList **apBins, XnUInt32 currBin,
  ...
  XnStatus Set(const TKey &key, const TValue &value) {
    ...
    for (typename TPairList::Iterator it = m_apBins[hash]->Begin();
         it != m_apBins[hash]->End(); ++it) {
  ...
  private:
  bool Find(const TKey &key, XnUInt32 &bin,
            typename TPairList::ConstIterator &currIt) const {
    xnl::HashCode hash = KeyManager::Hash(key);

    if (m_apBins[hash] != NULL) {
      for (typename TPairList::ConstIterator it = m_apBins[hash]->Begin();
           it != m_apBins[hash]->End(); ++it) {
        if (KeyManager::Compare(it->Key(), key) == 0) {
          bin = hash;
          currIt = it;
...
template <class TValue> class StringsNodeAllocator {
  public:
...
class StringsHash : public Hash<const XnChar *, TValue, StringsHashKeyManager,
                                StringsNodeAllocator<TValue>> {
  typedef Hash<const XnChar *, TValue, StringsHashKeyManager,
               StringsNodeAllocator<TValue>>
      Base;
  ...
  StringsHash(const StringsHash &other) : Base() { *this = other; }

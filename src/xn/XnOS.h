...
#include "Win32/XnOSWin32.h"
       ...
       XN_PLATFORM == XN_PLATFORM_ANDROID_ARM)
...
/** Allocate Z elements of Y type into the X pointer and make sure NULL wasn't
 * returned. */
#define XN_VALIDATE_CALLOC(x, y, z)                                            \
  x = (y *)xnOSCalloc(z, sizeof(y));                                           \
  XN_VALIDATE_ALLOC_PTR(x);

/** Allocate Y aligned bytes into the X pointer and make sure NULL wasn't
 * returned. */
#define XN_VALIDATE_ALIGNED_ALLOC(x, y, w)                                     \
  x = (y *)xnOSMallocAligned(sizeof(y), w);                                    \
  XN_VALIDATE_ALLOC_PTR(x);

/** Allocate Z aligned elements of Y type into the X pointer and make sure NULL
 * wasn't returned. */
...
/** Creates a new type object, validates that allocation succeeded, and
 * initializes the object (type must have an Init function). */
#if XN_PLATFORM_VAARGS_TYPE == XN_PLATFORM_USE_WIN32_VAARGS_STYLE
...
static void *operator new(size_t size, const XnChar *csFunction,
                          const XnChar *csFile, XnUInt32 nLine,
                          const XnChar *csAdditional) {
  void *p = xnOSMalloc(size);
  return xnOSLogMemAlloc(p, XN_ALLOCATION_NEW, size, csFunction, csFile, nLine,
                         csAdditional);
}
...
static void *operator new[](size_t size, const XnChar *csFunction,
                            ...
                            const XnChar *csAdditional) {
  void *p = xnOSMalloc(size);
  return xnOSLogMemAlloc(p, XN_ALLOCATION_NEW_ARRAY, size, csFunction, csFile,
                         nLine, csAdditional);
}
...
#define xnOSMalloc(nAllocSize)                                                 \
  xnOSLogMemAlloc(xnOSMalloc(nAllocSize), XN_ALLOCATION_MALLOC, nAllocSize,    \
                  __FUNCTION__, __FILE__, __LINE__, NULL)
#define xnOSMallocAligned(nAllocSize, nAlignment)                              \
  xnOSLogMemAlloc(xnOSMallocAligned(nAllocSize, nAlignment),                   \
                  XN_ALLOCATION_MALLOC_ALIGNED, nAllocSize, __FUNCTION__,      \
                  __FILE__, __LINE__, "Aligned to " XN_STRINGIFY(nAlignment))
#define xnOSCalloc(nAllocNum, nAllocSize)                                      \
  xnOSLogMemAlloc(xnOSCalloc(nAllocNum, nAllocSize), XN_ALLOCATION_CALLOC,     \
                  nAllocNum *nAllocSize, __FUNCTION__, __FILE__, __LINE__,     \
                  NULL)
#define xnOSCallocAligned(nAllocNum, nAllocSize, nAlignment)                   \
  xnOSLogMemAlloc(xnOSCallocAligned(nAllocNum, nAllocSize, nAlignment),        \
                  XN_ALLOCATION_CALLOC_ALIGNED, nAllocNum *nAllocSize,         \
                  __FUNCTION__, __FILE__, __LINE__,                            \
                  "Aligned to " XN_STRINGIFY(nAlignment))
#define xnOSFree(pMemBlock)                                                    \
  {                                                                            \
    xnOSLogMemFree(pMemBlock);                                                 \
    xnOSFree(pMemBlock);                                                       \
  }
#define xnOSFreeAligned(pMemBlock)                                             \
  {                                                                            \
    xnOSLogMemFree(pMemBlock);                                                 \
    xnOSFreeAligned(pMemBlock);                                                \
  }
...
#if XN_PLATFORM_VAARGS_TYPE == XN_PLATFORM_USE_WIN32_VAARGS_STYLE
#define XN_NEW(type, ...)                                                      \
  new (__FUNCTION__, __FILE__, __LINE__, XN_STRINGIFY(type)) type(__VA_ARGS__)
#elif XN_PLATFORM_VAARGS_TYPE == XN_PLATFORM_USE_GCC_VAARGS_STYLE
#define XN_NEW(type, arg...)                                                   \
  new (__FUNCTION__, __FILE__, __LINE__, XN_STRINGIFY(type)) type(arg)
#else
#define XN_NEW(type, arg)                                                      \
  new (__FUNCTION__, __FILE__, __LINE__, XN_STRINGIFY(type)) type(arg)
#endif
...
#define XN_NEW_ARR(type, count)                                                \
  new (__FUNCTION__, __FILE__, __LINE__,                                       \
       XN_STRINGIFY(count) " " XN_STRINGIFY(type)) type[count]

#endif
#endif

// Files
XN_C_API XnStatus XN_C_DECL xnOSCountFiles(const XnChar *cpSearchPattern,
                                           XnInt32 *pnFoundFiles);
XN_C_API XnStatus XN_C_DECL
xnOSGetFileList(const XnChar *cpSearchPattern, const XnChar *cpPrefixPath,
                XnChar cpFileList[][XN_FILE_MAX_PATH], const XnInt32 nMaxFiles,
                XnInt32 *pnFoundFiles);
...
XN_C_API XnStatus XN_C_DECL xnOSReadFile(const XN_FILE_HANDLE File,
                                         void *pBuffer, XnUInt32 *pnBufferSize);
...
XN_C_API XnStatus XN_API_DEPRECATED("Use xnOSSeekFile64() instead") XN_C_DECL
    xnOSSeekFile(const XN_FILE_HANDLE File, const XnOSSeekType SeekType,
...
XN_C_API XnStatus XN_C_DECL xnOSTellFile64(const XN_FILE_HANDLE File,
                                           XnUInt64 *nFilePos);
XN_C_API XnStatus XN_C_DECL xnOSTruncateFile64(const XN_FILE_HANDLE File,
                                               XnUInt64 nFilePos);
...
XN_C_API XnStatus XN_C_DECL xnOSDoesFileExist(const XnChar *cpFileName,
                                              XnBool *pbResult);
XN_C_API XnStatus XN_C_DECL xnOSDoesDirectoryExist(const XnChar *cpDirName,
                                                   XnBool *pbResult);
...
XN_C_API XnStatus XN_C_DECL
xnOSGetApplicationFilesDir(XnChar *cpDirName, const XnUInt32 nBufferSize);
XN_C_API XnStatus XN_C_DECL
xnOSGetApplicationLibDir(XnChar *cpDirName, const XnUInt32 nBufferSize);
#endif
/**
 * Strips the directory separator at the end of the specified path by directly
 * modifying the given string. Always returns XN_STATUS_OK.
 */
 ...
 * If the path component to append is an absolute path, the resulted path is
 * completely altered with it.
 *
 * @param	strDestPath					[in]	Buffer
 * that stores the original path to be operated.
 * @param	strPathComponentToAppend	[in]	Path component(s) to
 * append. Can be absolute or relative.
 * @param	nBufferSize					[in]	Size of
 * strDestPath.
 */
XN_C_API XnStatus XN_C_DECL
xnOSAppendFilePath(XnChar *strDestPath, const XnChar *strPathComponentToAppend,
                   const XnUInt32 nBufferSize);
/**
...
/**
 * Returns the absolute path of the module that includes the specified proc
 * address.
 *
 * @param	procAddr		[in]	Proc address contained by the
 * target module.
 * @param	strModulePath	[in]	Buffer to receive the absolute path of
 * the module. Must have the size of XN_FILE_MAX_PATH at least.
 */
XN_C_API XnStatus XN_C_DECL
xnOSGetModulePathForProcAddress(void *procAddr, XnChar *strModulePath);
...
XN_C_API XnStatus XN_C_DECL
xnOSTerminateThread(XN_THREAD_HANDLE *pThreadHandle);
...
XN_C_API XnStatus XN_C_DECL xnOSWaitAndTerminateThread(
    XN_THREAD_HANDLE *pThreadHandle, XnUInt32 nMilliseconds);
...
XN_C_API XnStatus XN_C_DECL
xnOSCreateNamedMutexEx(XN_MUTEX_HANDLE *pMutexHandle, const XnChar *cpMutexName,
...
// Critical Sections
XN_C_API XnStatus XN_C_DECL
xnOSCreateCriticalSection(XN_CRITICAL_SECTION_HANDLE *pCriticalSectionHandle);
XN_C_API XnStatus XN_C_DECL
xnOSCloseCriticalSection(XN_CRITICAL_SECTION_HANDLE *pCriticalSectionHandle);
XN_C_API XnStatus XN_C_DECL
xnOSEnterCriticalSection(XN_CRITICAL_SECTION_HANDLE *pCriticalSectionHandle);
XN_C_API XnStatus XN_C_DECL
xnOSLeaveCriticalSection(XN_CRITICAL_SECTION_HANDLE *pCriticalSectionHandle);
...
XN_C_API XnStatus XN_C_DECL xnOSCreateNamedEvent(XN_EVENT_HANDLE *pEventHandle,
                                                 ...
                                                 XnBool bManualReset);
XN_C_API XnStatus XN_C_DECL
xnOSCreateNamedEventEx(XN_EVENT_HANDLE *pEventHandle, const XnChar *cpEventName,
                       XnBool bManualReset, XnBool bAllowOtherUsers);
...
// Semaphores
XN_C_API XnStatus XN_C_DECL xnOSCreateSemaphore(
    XN_SEMAPHORE_HANDLE *pSemaphoreHandle, XnUInt32 nInitialCount);
...
XN_C_API XnStatus XN_C_DECL
xnOSCloseSemaphore(XN_SEMAPHORE_HANDLE *pSemaphoreHandle);

/**
 * Waits for a condition to be met. The condition is evaluated every time an
 * event is set.
 *
 * @param	EventHandle		[in]	The Event handle.
 * @param	nMilliseconds	[in]	A timeout in milliseconds to wait.
 * @param	pConditionFunc	[in]	A function that should be called to
 * evaluate condition.
 * @param	pConditionData	[in]	A cookie to be passed to the condition
 * functions.
 */
XN_C_API XnStatus XN_C_DECL
xnOSWaitForCondition(const XN_EVENT_HANDLE EventHandle, XnUInt32 nMilliseconds,
                     XnConditionFunc pConditionFunc, void *pConditionData);
...
XN_C_API XnStatus XN_C_DECL xnOSConnectSocket(XN_SOCKET_HANDLE Socket,
                                              XnUInt32 nMillisecsTimeout);
XN_C_API XnStatus XN_C_DECL xnOSSetSocketBufferSize(
    XN_SOCKET_HANDLE Socket, const XnUInt32 nSocketBufferSize);
...
XN_C_API XnStatus XN_C_DECL
xnOSReceiveNetworkBuffer(XN_SOCKET_HANDLE Socket, XnChar *cpBuffer,
                         XnUInt32 *pnBufferSize, XnUInt32 nMillisecsTimeout);
XN_C_API XnStatus XN_C_DECL xnOSReceiveFromNetworkBuffer(
    XN_SOCKET_HANDLE Socket, XnChar *cpBuffer, XnUInt32 *pnBufferSize,
 ...
 *
 * @param	strName			[in]	A machine-unique name that will
 * be used by other processes to open this block.
 * @param	nSize			[in]	The size of the buffer.
 * @param	nAccessFlags	[in]	Creation flags. Can contain
 * XN_OS_FILE_READ, XN_OS_FILE_WRITE or both.
 * @param	phSharedMem		[out]	A handle to the shared-memory
 * block.
 */
XN_C_API XnStatus XN_C_DECL xnOSCreateSharedMemory(
    const XnChar *strName, XnUInt32 nSize, XnUInt32 nAccessFlags,
    XN_SHARED_MEMORY_HANDLE *phSharedMem);

XN_C_API XnStatus XN_C_DECL xnOSCreateSharedMemoryEx(
    const XnChar *strName, XnUInt32 nSize, XnUInt32 nAccessFlags,
    XnBool bAllowOtherUsers, XN_SHARED_MEMORY_HANDLE *phSharedMem);

/**
 * Opens a shared memory block, and returns the address in which it was mapped
 * to the process' memory.
 *
 * @param	strName			[in]	A machine-unique name that will
 * be used by other processes to open this block.
 * @param	nAccessFlags	[in]	Creation flags. Must contain
 * XN_OS_FILE_READ, and optionally XN_OS_FILE_WRITE.
 * @param	phSharedMem		[out]	A handle to the shared-memory
 * block.
 */
XN_C_API XnStatus XN_C_DECL
xnOSOpenSharedMemory(const XnChar *strName, XnUInt32 nAccessFlags,
                     XN_SHARED_MEMORY_HANDLE *phSharedMem);

XN_C_API XnStatus XN_C_DECL xnOSOpenSharedMemoryEx(
    const XnChar *strName, XnUInt32 nAccessFlags, XnBool bAllowOtherUsers,
    XN_SHARED_MEMORY_HANDLE *phSharedMem);

/**
 * Closes a shared memory block.
 *
 * @param	hSharedMem		[in]	A handle to the block to be
 * closed.
 */
XN_C_API XnStatus XN_C_DECL
xnOSCloseSharedMemory(XN_SHARED_MEMORY_HANDLE hSharedMem);

/**
 * Gets the address in which the shared-memory block is mapped in this process.
 *
 * @param	hSharedMem		[in]	A handle to the shared memory
 * block.
 * @param	ppAddress		[out]	The address.
 */
XN_C_API XnStatus XN_C_DECL xnOSSharedMemoryGetAddress(
    XN_SHARED_MEMORY_HANDLE hSharedMem, void **ppAddress);
...
// Debug Utilities
XN_C_API XnStatus XN_C_DECL xnOSGetCurrentCallStack(XnInt32 nFramesToSkip,
                                                    XnChar **astrFrames,

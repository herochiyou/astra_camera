...
Vector3D SymmetricMatrix3x3::GetEigenValuesNoScaling() const {
  ...
  XnFloat p = (XnFloat)Math::ONE_SIXTH * (a00 * a00 + a11 * a11 + a22 * a22 +
                                          2 * (a01_sqr + a02_sqr + a12_sqr));
  XnFloat q = (XnFloat).5 * (a00 * (a11 * a22 - a12_sqr) - a11 * a02_sqr -
                             a22 * a01_sqr) +
              m_x01 * m_x02 * m_x12;
  XnFloat sqrt_p = Math::Sqrt(p), disc = p * p * p - q * q;
  XnFloat phi = (XnFloat)Math::ONE_THIRD *
                atan2(Math::Sqrt(Math::Max((XnFloat)0, disc)), q);
  XnFloat c = cos(phi), s = sin(phi);
  XnFloat sqrt_p_cos = sqrt_p * c,
          root_three_sqrt_p_sin = (XnFloat)Math::ROOT_THREE * sqrt_p * s;
  Vector3D lambda(m + 2 * sqrt_p_cos, m - sqrt_p_cos - root_three_sqrt_p_sin,
...
void SymmetricMatrix3x3::GetEigenVectors(const Vector3D &lambda,
                                         Matrix3x3 &eigenvectors,
...
void SymmetricMatrix3x3::GetEigenVector(XnFloat lambda, Vector3D &v) const {
  SymmetricMatrix3x3 COMat((m_x11 - lambda) * (m_x22 - lambda) - m_x12 * m_x12,
                           m_x12 * m_x02 - m_x01 * (m_x22 - lambda),
                           m_x01 * m_x12 - (m_x11 - lambda) * m_x02,
                           (m_x00 - lambda) * (m_x22 - lambda) - m_x02 * m_x02,
                           m_x01 * m_x02 - (m_x00 - lambda) * m_x12,
                           (m_x00 - lambda) * (m_x11 - lambda) - m_x01 * m_x01);
  XnFloat scales[3] = {
      Math::Sqr(COMat.m_x00) + Math::Sqr(COMat.m_x01) + Math::Sqr(COMat.m_x02),
  ...
  if (scales[0] > scales[1]) {
    if (scales[0] > scales[2]) {
      ...
      const XnFloat s =
          Math::OneOverSqrtHelper<XnFloat>::OneOverSqrt(scales[0]);
      v.Set(COMat.m_x00 * s, COMat.m_x01 * s, COMat.m_x02 * s);
      return;
    } else {
      // assert(i==2);
      const XnFloat s =
          Math::OneOverSqrtHelper<XnFloat>::OneOverSqrt(scales[2]);
...
// No scaling
void SymmetricMatrix3x3::SolveEigenProblemNoScaling(Vector3D &eigenvalues,
                                                    Matrix3x3 &eigenvectors,
                                                    XnFloat tolerance) const {
#ifdef USE_STABLE_EIGENVALUES
  EigenDecomposition(*this, eigenvectors, eigenvalues);
#else
  eigenvalues = GetEigenValuesNoScaling(); // This function takes care of the
                                           // scaling for us
...
// With scaling
void SymmetricMatrix3x3::SolveEigenProblemWithScaling(Vector3D &eigenvalues,
                                                      Matrix3x3 &eigenvectors,
                                                      XnFloat tolerance) const {
#ifdef USE_STABLE_EIGENVALUES
  XnFloat factor = GetScaleFactorForEigenValues(*this);
  SymmetricMatrix3x3 normalizedMatrix(factor * x00, factor * x01, factor * x02,
                                      factor * x11, factor * x12, factor * x22);
...
void SymmetricMatrix3x3::SolveEigenProblem(Vector3D &eigenvalues,
                                           Matrix3x3 &eigenvectors,
                                           XnFloat tolerance) const {
#ifdef ALWAYS_USE_SCALING_FOR_EIGENVALUES
...
// Uses scaling if ALWAYS_USE_SCALING_FOR_EIGENVALUES is defined
void SymmetricMatrix3x3::SolveSingleEigenProblem(XnInt32 i,
                                                 Vector3D &eigenvalues,
  ...
  SolveEigenProblem(
      ...
      tolerance); // This function takes care of the scaling for us
  eigenvector = eigenvectors.GetColumn(i);
#else
  eigenvalues =
      GetEigenValues(); // This function takes care of the scaling for us
  GetEigenVector(eigenvalues[i], eigenvector);
#endif
}

XnFloat SymmetricMatrix3x3::MaxAbs() const {
  return Math::Max(Math::MaxAbs(m_x00, m_x01, m_x02),
                   Math::MaxAbs(m_x11, m_x12), (XnFloat)Math::Abs(m_x22));
}
